...
> **Current-status override (2026-09-15):** a Fable full audit (live DB + CI history + 390px production screenshots) replaced the T1–T10 sequence with five ordered workstreams — W1 unstick the machine (mostly shipped 09-15), W2 honest savings on every card, W3 a feed for a Greek shopper's week + consent-free measurement, W4 a server-side profile + personal feed, W5 the vertical feed (design only until W3/W4 exist). It lives at the top of [CONTEXT.md](CONTEXT.md) with the measured audit and the six owner decisions. The "Do NOT until week-6 review" list below is relaxed only where that block says so (a server-side anonymous profile is W4; login/app stores stay deferred).

> **Current-status override (2026-09-02):** a Fable direction review set an ordered 6-week plan (T1–T10) at the top of [CONTEXT.md](CONTEXT.md). It supersedes the 06-26 note below. Work those tasks in order; the "Do NOT until week-6 review" list there (no 8th chain, no Capacitor/app stores, no login, no partner pitch, no more variant families) overrides any phase below that would start that work. Phase 4.7 (Capacitor) and Phase 5 (partner pitch) are explicitly gated on the week-6 traffic numbers. Groq is NOT billing-blocked — the default model was deprecated on 2026-07-17; T1 swaps it.
