...
**State when this was written (2026-09-16, ~04:00 Athens):** `origin/main` is at the commit that adds this text; working tree clean; **350 tests green**, `npm run lint` **0 errors and now blocking in CI**, `npm run build` green, `/` still static (revalidate 5m). **The whole 09-15 Step 1 list is done** (rows 18–22 plus small items 6a–6e); CI green on every pushed code commit; production verified for the essentials rail and the Greek heading font. Item 5 (server profile) is built and verified but **switched off** pending owner decision #7. Nothing is half-finished. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**
