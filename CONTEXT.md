...
**State when this was written (2026-09-16, ~02:30 Athens):** `origin/main` is at the commit that adds this text; working tree clean; **343 tests green**, `npm run build` green, `/` still static (revalidate 5m). Step 1 items 1–5 shipped on 09-16 (rows 18–22 below); the data passes for 1–4 ran live; item 5 is built and verified but **switched off** pending owner decision #7. Nothing is half-finished. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**

**Step 0 — verify before building anything (10 minutes, read-only).** If any of these is wrong, fix THAT first.
1. `git status` clean and `npm run test:run` → 343+ passing.
...
6. **NEXT: the small items below (6a–6e), then W4b** (personal feed; design W4 → 4b in «The plan»). W4b can be built against the `profiles` table while the flag is off, but will only have data after decision #7.
