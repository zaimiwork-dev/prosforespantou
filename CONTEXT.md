...
6a–6d. Small ones: ~~`.gitattributes`~~ **DONE 09-16 (`c4e9f02`; index was already all-LF, no content changed; `.ps1` keep CRLF)**; ~~fix the 5 AdminPanel lint errors and make CI lint blocking~~ **DONE 09-16 (`26f0604`; one was a real bug: the discount list's `useCallback` omitted `filterFeatured`, so the «featured» toggle reloaded with the old filter; lint now 0 errors / 4 warnings)**; ~~Greek serif for headings~~ **DONE 09-16 (commit after `26f0604`): Greek glyphs now render in Noto Serif Display (were Times New Roman); the stack is built in `src/app/layout.js` because `var(--font-serif)` bundles DM's metric fallback, which swallowed Greek, and Turbopack ignores `adjustFontFallback: false`. Also found: `Outfit` (used in sheets/inline styles) has no Greek subset either, so Greek there falls back to the system sans — harmless, not changed**; `pruneExpiredDatelessLeaflets()` runs a write on every supermarket page view (`src/app/supermarket/[id]/page.tsx`) — move it to the nightly job.
