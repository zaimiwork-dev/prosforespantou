...
**State when this was written (2026-09-16, ~19:45 Athens):** `origin/main` is at the docs commit below; working tree clean; **406 tests green**, `npm run lint` **0 errors** (4 warnings), CI runs the production build on every push. Decisions #2 and #7 are both **shipped and verified live** (row 23; row 22 — `profiles` is written on the live site and erased when consent is withdrawn). Step 1b item 1 is done (row 24: preferred stores boost the «Για σένα» rail instead of filtering it) and the UI is emoji-free (row 25). Step 1b is now done except for what needs the owner: rows 26 (pack-size guard), 27 (images shrunk to 800px WebP) and 28 (inline confirmations), with items 2b, 4 and the Kritikos provenance question **measured and closed without code**. **Step 1b is now closed** — rows 24-32 shipped it, and the three items that measurement killed are recorded with their numbers so nobody rebuilds them. **Next, in order: (a) OWNER — a custom image domain** (`images.prosforespantou.gr` → the R2 bucket): 97% of live offer images come from Cloudflare's public `r2.dev` host, which Cloudflare rate-limits and asks you not to use in production, and which some Greek resolvers blackhole (this dev machine's own DNS answers 127.0.0.1 for it). One DNS record on the owner's side, then `R2_PUBLIC_URL` plus a one-off UPDATE over `discounts.image_url` / `products.image_url`; the hostname is already allowed in `next.config.mjs`. **(b) OWNER — decision #1** (consent-free measurement), still the single biggest unlock: every event from row 19 fires only for visitors who accepted cookies, so the ranking work above is being tuned blind. **(c)** With neither answered, the best unblocked work is W2/W5 from the audit below, not more personalization — the ranker is now ahead of the data it is fed. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**
