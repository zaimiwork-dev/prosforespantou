...
**State when this was written (2026-09-16, ~13:00 Athens):** `origin/main` is at the docs commit below; working tree clean; **380 tests green**, `npm run lint` **0 errors** (4 warnings), CI runs the production build on every push. Decisions #2 and #7 are both **shipped and verified live** (row 23; row 22 — `profiles` is written on the live site and erased when consent is withdrawn). Step 1b item 1 is done (row 24: preferred stores boost the «Για σένα» rail instead of filtering it) and the UI is emoji-free (row 25). Step 1b item 2 is done as well (row 26: the pack-size guard; the shelf-outlier half was measured and rejected — see the item). **Next: Step 1b item 3** — the product images (the r2.dev host and the missing resize), which needs one owner decision on a DNS record. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**
...
3. **Product images — two real risks found 09-16 while chasing «kritikos has no pics».** The images themselves are fine: every sampled object exists in the R2 bucket, the live Κρητικός page renders them, and a fetch of each URL returns a real JPEG. What is fragile is how they are served. (a) **97% of live offer images (12,495 of 12,905) come from Cloudflare's public `pub-….r2.dev` host** — every chain except Σκλαβενίτης. Cloudflare rate-limits that host and says not to use it in production, AND some resolvers blackhole it: **this dev machine's own DNS answers 127.0.0.1 for it**, which would hide every product photo from a visitor on that resolver. The fix is a custom domain on the bucket (`images.prosforespantou.gr` → R2, then `R2_PUBLIC_URL` plus a one-off UPDATE over `discounts.image_url` / `products.image_url`); it needs the owner's DNS, and the hostname is already allowed in `next.config.mjs`. (b) **Nothing is resized.** With the Vercel optimizer off (`images.unoptimized`, Hobby-tier quota), phones download the originals: Κρητικός 1080px JPEGs, My Market 1000px PNGs — **~5-6 MB for one screen of 48 cards**, displayed at ~170px. Μασούτης (900px webp, 22KB median) shows what the same page could weigh. `sharp` is already a dependency, so resizing at mirror time in `src/scripts/lib/mirror-images.mjs` is the cheap fix; note the object path is derived from the source URL's extension, so a webp re-encode re-downloads each image once.
...
7. **alert()/confirm() → inline confirmations** in the shopping list and admin (audit item G; `ShoppingList.js` «Εκκαθάριση λίστας;» uses `confirm`).
