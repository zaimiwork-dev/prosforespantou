...
**State when this was written (2026-09-17, ~23:45 Athens):** `origin/main` is at the docs commit below; working tree clean; **414 tests green**, `npm run lint` **0 errors** (4 warnings), CI runs the production build on every push. Decisions #2 and #7 are both **shipped and verified live** (row 23; row 22 — `profiles` is written on the live site and erased when consent is withdrawn). Step 1b item 1 is done (row 24: preferred stores boost the «Για σένα» rail instead of filtering it) and the UI is emoji-free (row 25). Step 1b is now done except for what needs the owner: rows 26 (pack-size guard), 27 (images shrunk to 800px WebP) and 28 (inline confirmations), with items 2b, 4 and the Kritikos provenance question **measured and closed without code**. **Step 1b is closed (rows 24-32) and both open owner decisions were answered on 09-17.** **Decision #1 (measurement) = YES** → row 34 shipped the anonymous, storage-free layer. **It is still OFF in production: set `NEXT_PUBLIC_ANON_ANALYTICS=1` in Vercel (Production, Config not Secret) and redeploy.** Nothing else is needed; the legal pages and the banner switch on the same flag. **Row 35 adds a SECOND, separate flag `NEXT_PUBLIC_VISIT_ID` (tab-scoped visit id — real device storage, pending the owner's lawyer). Do NOT flip that one until someone has run the browser check it never got: start `npm run dev` with both flags and confirm a `v-` row lands and `consent_choice` fires.** **Image domain = watch now, move before promoting the site** → row 33 shipped the daily watchdog. The custom domain itself is NOT just a DNS record: `prosforespantou.gr` runs on **Vercel nameservers** (`ns1/ns2.vercel-dns.com`) and Cloudflare R2 custom domains require the zone to live on Cloudflare, so it means moving DNS hosting to Cloudflare and recreating the Vercel records. Do it before any traffic push, then set `R2_PUBLIC_URL` (Vercel + GH secrets) and run one UPDATE over `discounts.image_url` / `products.image_url` (~42k rows); the hostname is already allowed in `next.config.mjs`. **Next, once the flag is on:** let it run a week, then answer the questions the ranker has never been able to answer — click-through by slot position, cheapest-in-cluster rows vs the rest, and `context='mine'` vs `'other'` (does the store boost earn its place?). Tune rows 24/32 from that, not from argument. In the meantime the best unblocked work is W2/W5 from the audit below. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**
