...
## ⚡ Pick up here (2026-06-12 night — user bug-report session: 5 slices SHIPPED locally, ⚠️ NOT PUSHED)

**User reported the expected bugs + asked the big questions; all answered and 5 commits landed on local main (`9e4cf8a`→docs). ⚠️ `git push origin main` was DENIED to the agent by the permission system — USER MUST PUSH, then dispatch the mapping audit (below).**
...
**USER ACTIONS queued:** ① `git push origin main` ② dispatch + eyeball + apply the mapping audit ③ add the 2 SUPABASE env vars in Vercel ④ check ab.gr/fylladia in a browser.
**Still open from before:** Bazaar+Γαλαξίας recon; Lidl OCR cleanup; admin-panel UX rethink (user finds Review/Λίστα/Library confusing — explained, redesign deferred); repo-root scratch txt hygiene.
