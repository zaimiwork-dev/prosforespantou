...
2. **Per-family penalty in `recompute-hotness.mjs`** if the /deals first screen still shows several Misko pastas on a phone (see 6e). Measure with `.local-scratch/deals-rendered-families.mjs` before and after.
3. **Kritikos provenance question** (Step 0 item 3), read-only investigation first.
4. **alert()/confirm() → inline confirmations** in the shopping list and admin (audit item G; `ShoppingList.js` «Εκκαθάριση λίστας;» uses `confirm`).
5. After decision #1 (consent-free measurement): build 3c; after decision #2: render «κανονικά ~Y€» from `baseline_price` / `implied_percent` in `DiscountCard.js` for `implied_percent >= 5`.
...
7. **(added 09-16) Switch on the anonymous server profile (W4a).** Today /cookies and /aporrito say preferences «δεν αποστέλλονται σε εμάς». The feature copies stores, departments, favorites, list lines, learned interests and text size to the `profiles` table under a random `pp-profile-id`, only after cookie consent, and deletes the copy when consent is withdrawn. The replacement wording is already written and appears automatically when `NEXT_PUBLIC_PROFILE_SYNC=1` — read it in `src/app/cookies/page.js` and `src/app/aporrito/page.js` (search `PROFILE_SYNC`). Yes = set the env var in Vercel and redeploy. It is what makes W4b (personal feed) and W5 (reels) able to learn.
