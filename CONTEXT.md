...
3. **Open question, not yet investigated:** the Kritikos catalog walk `34994550427` finished green (8,061 items, healthy IngestRun) but `select matched_via, count(*) from chain_product_mappings where supermarket='kritikos' group by 1` still shows **5,874 null / 2,724 barcode** (only 44 converted). The 09-15 expectation that the walk would stamp most of them was wrong. Likely the null rows are offer-side mappings whose `chainItemcode` never appears in the catalog walk, or `ingestCatalog` only stamps mappings it creates. Read `src/scripts/lib/ingest-catalog.mjs` (search `matchedVia`) before assuming a bug.
...
4. **Per-family penalty in `recompute-hotness.mjs`** if the /deals first screen still shows several Misko pastas on a phone (see 6e). Measure with `.local-scratch/deals-rendered-families.mjs` before and after.
