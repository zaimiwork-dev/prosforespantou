...
**State when this was written (2026-09-16, ~13:00 Athens):** `origin/main` is at the docs commit below; working tree clean; **380 tests green**, `npm run lint` **0 errors** (4 warnings), CI runs the production build on every push. Decisions #2 and #7 are both **shipped and verified live** (row 23; row 22 — `profiles` is written on the live site and erased when consent is withdrawn). Step 1b item 1 is done (row 24: preferred stores boost the «Για σένα» rail instead of filtering it) and the UI is emoji-free (row 25). **Next: Step 1b item 2** — the two data bugs (Fairy 19τεμ/30τεμ sharing one Product; a single outlier shelf snapshot standing as the «normal» price). **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**
