...
| 7 | My Market % missing on most items | ⛔ won't-fix (2026-06-09) — **premise was wrong** | The earlier note claimed the analytics JSON's `price` is the *regular* price. Verified on live data (`/offers?page=2`, 25/25 on-offer items): `analytics.price === teaser display price` exactly — it's the OFFER price, not the regular price. The product detail page likewise exposes only `price` (no strikethrough, no `oldPrice`/`regularPrice`, no "discount" keyword). My Market simply publishes single offer prices (ΜΟΝΟ-style, like Sklavenitis); there is no regular price to compute a % from. These items are now value-ranked via hotScore (KVI/brand/mechanic), so the underlying "looks value-less" concern is addressed without a fake %. Theoretical-but-not-worth-it fallback: infer originalPrice from PriceSnapshot history (unreliable for unmatched/ΜΟΝΟ items). |
