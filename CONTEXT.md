...
## ⚡ Pick up here (2026-09-15 — Fable full audit: where we are, what shipped, what to do next; updated 2026-09-16 after Step 1 items 1–4)
...
**State when this was written (2026-09-16, ~01:30 Athens):** `origin/main` is at the commit that adds this text; working tree clean; **337 tests green**, `npm run build` green, `/` still static (revalidate 5m). Step 1 items 1–4 shipped on 09-16 (rows 18–21 below) and their data passes ran live. Nothing is half-finished. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**

**Step 0 — verify before building anything (10 minutes, read-only).** If any of these is wrong, fix THAT first.
1. `git status` clean and `npm run test:run` → 337+ passing.
...
5. **W4a server-side profile — NEXT** (new `profiles` table, `src/actions/sync-profile.ts`, `src/lib/profile-sync.js`): design is W4 → 4a in «The plan» below. Login stays off. The events from row 19 already record declared stores/departments (`store_select`, `filter` with `page='onboarding'|'preferences'`), so the profile can be seeded from them.
6. Small ones, any time: `.gitattributes` (`* text=auto eol=lf`) in its own commit; fix the 5 `react-hooks/set-state-in-effect` errors in `src/components/AdminPanel.js` and then remove `continue-on-error` from the lint step in `ci.yml`; give `DM_Serif_Display` a fallback that has Greek (headings render in a system serif today — `src/app/layout.js`); `pruneExpiredDatelessLeaflets()` runs a write on every supermarket page view (`src/app/supermarket/[id]/page.tsx`) — move it to the nightly job.
7. New from 09-16, small: the `/deals` hot sort has no family cap, so after row 21 it can show several Misko pastas in a row (the homepage rail caps families; `/deals` does not). Consider `capPerFamily` on the first page only.

**Step 2 — blocked on the owner (do not start; ask if unclear):** the six decisions in «⛔ Owner decisions» below. Decision #1 (consent-free measurement) unblocks the most: the events from row 19 fire only for visitors who accepted cookies. Decision #2 now has its data: 7,458 ΜΟΝΟ offers carry `baseline_price` / `implied_percent` (6,768 below shelf); a card line «κανονικά ~Y€» is a render-only change in `src/components/DiscountCard.js`. **AB's ΜΟΝΟ offers all sit exactly at shelf price** (227 baselines, none below): at AB the saving is the mechanic (1+1 etc.), so a card line must stay silent there by construction (`implied_percent <= 0`).
...
### Shipped 2026-09-16 (Step 1 items 1–4; all on `origin/main`)
...
**W4 — Personalization that survives a device** (≈ 5 days; **4a is NEXT** — Step 1 item 5)
