...
**State when this was written (2026-09-16, ~13:00 Athens):** `origin/main` is at the docs commit below; working tree clean; **380 tests green**, `npm run lint` **0 errors** (4 warnings), CI runs the production build on every push. Decisions #2 and #7 are both **shipped and verified live** (row 23; row 22 — `profiles` is written on the live site and erased when consent is withdrawn). Step 1b item 1 is done (row 24: preferred stores boost the «Για σένα» rail instead of filtering it) and the UI is emoji-free (row 25). Step 1b is now done except for what needs the owner: rows 26 (pack-size guard), 27 (images shrunk to 800px WebP) and 28 (inline confirmations), with items 2b, 4 and the Kritikos provenance question **measured and closed without code**. **Next: (a) owner decision — a custom image domain** (`images.prosforespantou.gr` → the R2 bucket) so product photos stop depending on Cloudflare's public `r2.dev` host, which is rate-limited and blackholed by some Greek resolvers; (b) the AdminPanel clean-up (emoji + its five `confirm()` calls); (c) decision #1 (consent-free measurement), which still unblocks the most. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**
...
1. ~~**W4b personal feed — stores as a boost**~~ **DONE 09-16, row 24** (`src/lib/personal-feed.ts` + `HomeClient.js`). What is left of W4b: reading `profiles` server-side (now unblocked — decision #7 is live), favorites' live hits as the first rows, and the same ranking on `/deals`. The rail is still built in the browser from three `getActiveDeals` calls.
