...
**State when this was written (2026-09-15, ~20:00 Athens):** `origin/main` is at the commit that adds this section; working tree clean; **304 tests green**, `npm run build` green, CI gate (`.github/workflows/ci.yml`) green on the last push; production deploy verified at 390px. 20 commits landed today (rows 1–17 below). Nothing is half-finished. Nothing is uncommitted.

**Step 0 — verify before building anything (10 minutes, read-only).** If any of these is wrong, fix THAT first; the plan assumes them.
1. `git status` clean and `npm run test:run` → 304+ passing.
2. Last night's `resolvers` run (GitHub Actions → "Scrape supermarket chains", schedule `0 4 * * *`): each chain's step prints `pending rows to process: N (M back a live productless offer)` with M > 0 for masoutis/lidl/mymarket, and `errors: 0`. If M = 0 everywhere, the priority query in `src/scripts/resolve-pending-matches.mjs` (search `prioritized`) broke.
3. The Kritikos catalog walk dispatched on 09-15 (`gh run view 34994550427`) finished green and `select matched_via, count(*) from chain_product_mappings where supermarket='kritikos' group by 1` shows most rows as `barcode` (was 5,918 null). If it failed, read its log — it is the first time `kritikos-canonical-scraper.mjs` runs through `ingestCatalog`.
...
5. The 08:00 UTC `pipeline-health` run is green, or red naming only `sklavenitis/*` (the laptop dependency) — anything else red is new and must be read.
6. Read-only DB snapshot for numbers: `node .local-scratch/fable-state.mjs` (it is gitignored; if missing, the queries are trivial: active offers by chain/source, last ingest run per feed, pending_matches by chain, products with barcode).

**Step 1 — the ordered build list (no owner decision needed for any of these).** Each is independently shippable; commit + push each with tests and build green; write a dated line in the table below when done.
1. **W3b homepage restructure** (`src/components/HomeClient.js`, `CategoryGrid.js`, `SupermarketTiles.js`, `globals.css`): (a) new first rail «Τα βασικά της εβδομάδας» = one card per KVI staple (γάλα, καφές, ελαιόλαδο, φέτα, απορρυπαντικό, χαρτί υγείας, πάνες, ζυμαρικά, αναψυκτικά, μπύρα) showing the cheapest chain today — new cached action `getWeeklyEssentials` in `src/actions/get-active-deals.ts` reusing `KVI_TIERS` terms from `src/lib/hotness.ts` + `dedupeDeals({crossChain:true})` + `capPerFamily`; (b) category grid trimmed to 8 big tiles + «Περισσότερα»; (c) chain tiles as one horizontal strip; (d) newsletter box out of the first scroll. Acceptance: 390px screenshot of `/` shows real staples with prices above the fold; `npm run build` shows `/` still static (revalidate 5m).
2. **W3d event coverage** (`src/actions/track-event.ts`, `src/lib/track.js`, `src/components/{SearchPage,DealsClient,OfferDetails,ShoppingList,SupermarketClient,DiscountCard}.js`): fire the 5 declared-but-dead events (`search` with query + result count, `filter`, `store_select`, `favorite`/unfavorite, `outbound_click`) plus `list_remove` and batched `impression`s (IntersectionObserver ≥50% for ≥1 s, flushed every 5 s, cap 200/session). Needs DDL: `click_events ADD COLUMN position int, page text, query text, result_count int, dedup_key text; CREATE UNIQUE INDEX ... (dedup_key) WHERE dedup_key IS NOT NULL; CREATE INDEX ... (session_id, created_at)` — apply over the pooled port with a `.local-scratch/*.mjs` script like `add-judged-at.mjs` (pattern in row 2), mirror in `prisma/schema.prisma`, `npx prisma generate`, delete `.next`. Events stay behind the consent gate until decision #1.
3. **W2a data side** (`src/lib/price-verdict.ts`, `src/scripts/recompute-price-verdicts.mjs`): for ΜΟΝΟ offers with a same-chain `PriceSnapshot kind='normal'` and a fresh feed (`freshShelfChains` from `src/lib/feed-freshness.ts`), persist `Discount.baselinePrice` + `baselineAt` (DDL: two nullable columns) and `impliedPercent`. Do NOT render it on cards until decision #2; the detail view already shows the median «Κανονική τιμή» in `PriceHistory.js`.
4. **W3a remainder**: `is_cheapest_in_cluster` boolean on `discounts`, computed in `src/scripts/recompute-comparison-counts.mjs` (extend `comparisonChainCount` in `src/lib/comparison-count.ts` to return prices), +10 in `computeHotScore` for the cheapest row. Then re-run the top-30 dump (a read-only script; see row 13's method) before committing.
5. **W4a server-side profile** (new `profiles` table, `src/actions/sync-profile.ts`, `src/lib/profile-sync.js`): design is in the implementation notes below. Login stays off.
...
**Step 2 — blocked on the owner (do not start; ask if unclear):** the six decisions in «⛔ Owner decisions» below. Decision #1 (consent-free measurement) unblocks the most: build 3c only after a yes.

**How we work here (conventions that bit us; all in CLAUDE.md, repeated for non-Claude tools):** components are `.js`, libs/actions `.ts`, no TSX migration, vanilla CSS in `src/app/globals.css`. Every DB-touching `.mjs` script loads dotenv BEFORE importing prisma. `Discount.source` isolation and soft-delete only. Never auto-create Products from a matcher verdict. Groq and ab.gr are IP-blocked from the dev PC → pipeline verification is `gh workflow run scrape-chains.yml -f chain=<job> -f limit=N` then `gh run view <id> --log`. Port 5432 is unreachable from the dev PC → DDL over the pooled 6543 URL with `ADD COLUMN IF NOT EXISTS`, then `npx prisma generate` and delete `.next`. Before every commit: `npm run test:run` and `npm run build`. Verify UI on production at 390px (`npx playwright screenshot --viewport-size=390,844 <url> out.png`); `npm run build` passing is necessary, not sufficient. Measure before assuming — three "facts" in older notes were wrong today. **End every session by rewriting this START HERE section** so the next one (any model) knows exactly where things stand.

### Status in one table (2026-09-15, end of day)
...
**Still running when this was written:** the Kritikos catalog walk `34994550427` (a ~7k-product crawl, 1–2 h). Check it in Step 0.
