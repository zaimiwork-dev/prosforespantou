...
## ⚡ Pick up here (2026-09-15 — Fable full audit: where we are, what shipped, what to do next; updated 2026-09-16 after Step 1 items 1–5)
...
### ▶ NEXT SESSION — START HERE (written for any model: Claude, GPT, whoever)

**Update 2026-09-23 (read first; rows 38-40).** AB offers were at **zero from 09-17 to 09-23** and are fixed (row 38: AB now hides about half of each promotions page after paging; the adapter asks 50 per page and walks past empty pages; 275 live again). Sunday's canonical runs worked (barcodes 19,687 → 25,247). Σκλαβενίτης's "167" is the known Wolt GTIN loss, not a bug. **Traffic is ~0** (19 events from 1 id since 09-19), so the ranker-tuning plan below has nothing to tune from until there are visitors; distribution is the constraint. The image-domain move is built and waiting on the owner (row 39) — **Image domain, owner steps, in order:** (1) create a free Cloudflare account → Add site `prosforespantou.gr` → Free plan; check the imported records against row 39's inventory (apex A `216.198.79.1` + `64.29.17.1`, `www`, wildcard `*`, CAA) and set every Vercel record to **DNS only (grey cloud)**. (2) Only then, at the .gr registrar, replace `ns1/ns2.vercel-dns.com` with the two Cloudflare nameservers; both sets serve identical records during the switch, so there is no downtime if step 1 was complete. (3) When Cloudflare shows the zone **Active**: R2 → the bucket → Settings → Custom Domains → connect `images.prosforespantou.gr`. (4) Dry run: `NEW_IMAGE_BASE=https://images.prosforespantou.gr node src/scripts/rewrite-image-host.mjs` — must print `✓ every sampled photo is served by the new host`. (5) Set `R2_PUBLIC_URL=https://images.prosforespantou.gr` in GitHub secrets AND Vercel Production, redeploy. (6) Same command with `APPLY=1`; run it once more after the next nightly scrape to catch rows written in between. Do it before any traffic push.

**State when this was written (2026-09-17, ~23:45 Athens):** `origin/main` is at the docs commit below; working tree clean; **414 tests green**, `npm run lint` **0 errors** (4 warnings), CI runs the production build on every push. Decisions #2 and #7 are both **shipped and verified live** (row 23; row 22 — `profiles` is written on the live site and erased when consent is withdrawn). Step 1b item 1 is done (row 24: preferred stores boost the «Για σένα» rail instead of filtering it) and the UI is emoji-free (row 25). Step 1b is now done except for what needs the owner: rows 26 (pack-size guard), 27 (images shrunk to 800px WebP) and 28 (inline confirmations), with items 2b, 4 and the Kritikos provenance question **measured and closed without code**. **Step 1b is closed (rows 24-32) and both open owner decisions were answered on 09-17.** **Decision #1 (measurement) = YES** → row 34 shipped the anonymous, storage-free layer. **`NEXT_PUBLIC_ANON_ANALYTICS=1` is LIVE in Vercel Production since 09-17** (verified: production `/cookies` serves the anonymous-measurement heading). **Two further flags are built and verified but still OFF, both waiting on the owner's lawyer: `NEXT_PUBLIC_VISIT_ID`** (tab-scoped visit id — real device storage, row 35/36) **and `NEXT_PUBLIC_CONSENT_ACCEPT_FIRST`** (accept-first banner, row 37 — read that row before touching it; it is the one that can invalidate the consent everything else rests on). **Image domain = watch now, move before promoting the site** → row 33 shipped the daily watchdog. The custom domain itself is NOT just a DNS record: `prosforespantou.gr` runs on **Vercel nameservers** (`ns1/ns2.vercel-dns.com`) and Cloudflare R2 custom domains require the zone to live on Cloudflare, so it means moving DNS hosting to Cloudflare and recreating the Vercel records. Do it before any traffic push, then set `R2_PUBLIC_URL` (Vercel + GH secrets) and run one UPDATE over `discounts.image_url` / `products.image_url` (~42k rows); the hostname is already allowed in `next.config.mjs`. **Next, once the flag is on:** let it run a week, then answer the questions the ranker has never been able to answer — click-through by slot position, cheapest-in-cluster rows vs the rest, and `context='mine'` vs `'other'` (does the store boost earn its place?). Tune rows 24/32 from that, not from argument. In the meantime the best unblocked work is W2/W5 from the audit below. **Rule from the owner (09-16): update this section in the same step as every item you finish, not at the end of the session.**

**Step 0 — verify before building anything (10 minutes, read-only).** If any of these is wrong, fix THAT first.
1. `git status` clean, `npm run test:run` → 350+ passing, `npm run lint` → 0 errors.
2. The nightly `resolvers` job (GitHub Actions → "Scrape supermarket chains", schedule `0 4 * * *`) on 09-16 and later: each resolver step prints `pending rows to process: N (M back a live productless offer)` with M > 0 for masoutis/lidl/mymarket and `errors: 0`; the recompute steps now run in the order **verdicts → comparison counts → hotScore** (row 21) and the gate lists 10 outcomes. `recompute-price-verdicts` prints a `baselines: N ΜΟΝΟ offers` line (**~2.7k since the 09-16 honesty guards**, not 7.4k); `recompute-comparison-counts` prints `cheapest in their cluster: N` (~2.9k).
3. ~~Open question: Kritikos provenance~~ **ANSWERED 09-16 — no bug, nothing to fix.** The numbers had already flipped by the time anyone looked: `chain_product_mappings` for kritikos is now **5,784 `barcode` / 2,814 null** (the 09-15 note said 5,874 null / 2,724 barcode), so the re-stamping fix did work. Of the 2,814 still without provenance, **2,808 back no live offer at all** — dormant SKUs from past offers, median age 107 days, which neither an offer run nor a catalog walk can stamp because the chain no longer lists them. The 6 that do back a live offer resolve by barcode to exactly the product they are mapped to, so they stamp on the chain's next offer run. Provenance heals as SKUs reappear; the null count is history, not breakage.
4. After Sunday 09-20: `select count(barcode) from products` ≈ 25,000 (was 19,687) and `select chain, scraped_items from ingest_runs where source='wolt'` has masoutis/ab/mymarket/sklavenitis rows. If not, the four `*-canonical` Sunday jobs did not run — check `gh run list`.
5. The 08:00 UTC `pipeline-health` run is green, or red naming only `sklavenitis/*` (the laptop dependency). **Sklavenitis shelf prices were last confirmed by a catalog run on 09-06**; the freshness window is 10 days, so from ~09-17 its baselines, shelf rows and «cheapest» flags drop out until the laptop catalog task completes a run (its time limit was raised to 6 h on 09-15). That is the gate working, not a bug.
6. Production at 390px: `/` shows «Τα βασικά της εβδομάδας» with 10 staple cards above «Κορυφαίες προσφορές» (verified live 09-16), 8 category tiles + «Περισσότερες», chains as one strip.

**Step 1 — the ordered build list (no owner decision needed).** Commit + push each with tests and build green, then update THIS section and add a row to the 09-16 table in the same step.
1. ~~W3b homepage restructure~~ **DONE 09-16, row 18 (`58928c5`).**
2. ~~W3d event coverage~~ **DONE 09-16, row 19 (`c885e31`).**
3. ~~W2a data side~~ **DONE 09-16, row 20 (`9de6ae0`).** Rendering on cards still waits on owner decision #2.
4. ~~W3a remainder (cheapest in cluster)~~ **DONE 09-16, row 21 (`71023bf`).**
5. ~~W4a server-side profile~~ **DONE 09-16, row 22 (`5cd35f3`) — shipped OFF behind `NEXT_PUBLIC_PROFILE_SYNC`.** The published privacy policy promises preferences never leave the device, so turning it on is owner decision #7. To enable after a yes: set `NEXT_PUBLIC_PROFILE_SYNC=1` in Vercel (Production) and redeploy; the /cookies and /aporrito wording switches with the same flag. Nothing reads `profiles` yet (that is W4b).
6. ~~Small items 6a–6e~~ **ALL DONE 09-16** (details below).
6a–6d. Small ones: ~~`.gitattributes`~~ **DONE 09-16 (`c4e9f02`; index was already all-LF, no content changed; `.ps1` keep CRLF)**; ~~fix the 5 AdminPanel lint errors and make CI lint blocking~~ **DONE 09-16 (`26f0604`; one was a real bug: the discount list's `useCallback` omitted `filterFeatured`, so the «featured» toggle reloaded with the old filter; lint now 0 errors / 4 warnings)**; ~~Greek serif for headings~~ **DONE 09-16 (`c4c8c11`): Greek glyphs now render in Noto Serif Display (were Times New Roman); the stack is built in `src/app/layout.js` because `var(--font-serif)` bundles DM's metric fallback, which swallowed Greek, and Turbopack ignores `adjustFontFallback: false`. Also found: `Outfit` (used in sheets/inline styles) has no Greek subset either, so Greek there falls back to the system sans — harmless, not changed**; ~~leaflet prune off the page view~~ **DONE 09-16 (`7048866`): the supermarket page now only hides expired dateless leaflets when reading; the delete runs nightly in `scrape-chains.yml` (step `prune_leaflets`, inside the gate) and when an admin opens the leaflet list. It had also been a public server action with no admin check (exported from a `'use server'` file) — no longer exported.**
6e. ~~`/deals` family repetition~~ **DONE 09-16 (`64e8276`), with a measured, modest gain.** The default view already rotates departments (`interleaveByCategory`); the new `spreadFamilies` (in `src/lib/deal-family.ts`) spaces same-brand rows without dropping any. Rendered order, measured with `.local-scratch/deals-rendered-families.mjs`: first 72 rows, cards with a same-family card in the previous 3 went 11 → 3; page 1 alone barely changes (its tail is Misko ×3 because nothing else is left to swap in, and scrolling there loads page 2, which re-spreads). The root cause is ranking density: six Misko pastas are all flagged cheapest (row 21). If that still reads badly on a phone, the next lever is a per-family penalty inside `recompute-hotness.mjs`, not more reordering.

**Step 1b — what to build next (new list, 09-16; no owner decision needed unless noted).** In order:
1. ~~**W4b personal feed — stores as a boost**~~ **DONE 09-16, rows 24 and 32.** The rail (`src/lib/personal-feed.ts` + `HomeClient.js`) and now `/deals` too, where the boost had to move to the server because the cheaper chain is not in the loaded window at all. **The other two W4b ideas were looked at and deliberately not built:** (a) *favorites' live hits as the first rows of «Για σένα»* — `FavoritesRow` already renders «Τα αγαπημένα σου» ABOVE it on the same screen, so this would show the same card twice; (b) *reading `profiles` server-side* — the profile id lives in `localStorage`, so a server render cannot know whose it is without a cookie (which would make the homepage dynamic), and the device already holds everything the ranker needs. Server-side reading buys cross-device continuity, which needs a login that does not exist yet. Revisit it with accounts (PHASES Phase 10), not before. Also not built: a declared-department boost inside `/deals` — the homepage rail already is that feed, and `/deals` has department chips one tap away.
2. ~~**Two data bugs found while shipping row 23**~~ **(a) DONE 09-16, row 26** — the shared-Product case is now guarded at the comparison layer by `statedPieces`, and the 42 products whose live offers disagree on piece count (21 with a 1.5×+ price spread) surface in `audit-mappings.mjs` as `packOnly` (147 mappings) for the Review Queue. Splitting those Product records is still manual. **(b) INVESTIGATED AND REJECTED 09-16 — do not build the median rule.** The plan was «ignore a shelf reading >1.5× the median of that chain's other observations». Measured over every product backing a live offer: only 1,371 of 10,294 chain/product series have 3+ readings, 25 of them have a latest reading >1.5× the median — and reading them one by one, they are **not misreads**: they are the real shelf price returning after a promotion (AB: 5.18 on 13/09, 2.98 on 06/09, 2.99 on 26/07, 5.18 on 14/06 — the promo price is what the catalog feed reports as «normal», and `normal` snapshots are never flagged `is_discounted`, so we cannot tell them apart from the flag). The median rule would throw away the CURRENT shelf price and keep the promo one. The «Belle Τηγάνι» case that motivated it (mymarket: normal 17.98 on 16/08, normal 7.19 on 09/08, offer 6.50 today) is indistinguishable from a genuine deep cookware promotion; the 50% display cap already keeps it off the card. Leave the guards as they are until the feeds tell us which readings are promotional.
3. **Product images — two real risks found 09-16 while chasing «kritikos has no pics».** The images themselves are fine: every sampled object exists in the R2 bucket, the live Κρητικός page renders them, and a fetch of each URL returns a real JPEG. What is fragile is how they are served. (a) **97% of live offer images (12,495 of 12,905) come from Cloudflare's public `pub-….r2.dev` host** — every chain except Σκλαβενίτης. Cloudflare rate-limits that host and says not to use it in production, AND some resolvers blackhole it: **this dev machine's own DNS answers 127.0.0.1 for it**, which would hide every product photo from a visitor on that resolver. **STILL OPEN — this one needs the owner.** The fix is a custom domain on the bucket (`images.prosforespantou.gr` → R2, then `R2_PUBLIC_URL` plus a one-off UPDATE over `discounts.image_url` / `products.image_url`); it needs the owner's DNS, and the hostname is already allowed in `next.config.mjs`. (b) ~~**Nothing is resized.**~~ **DONE 09-16, row 27.** For the record, what it was: With the Vercel optimizer off (`images.unoptimized`, Hobby-tier quota), phones download the originals: Κρητικός 1080px JPEGs, My Market 1000px PNGs — **~5-6 MB for one screen of 48 cards**, displayed at ~170px. Μασούτης (900px webp, 22KB median) shows what the same page could weigh. New images are downscaled on the way in, and `shrink-mirrored-images.mjs` rewrites the ones already stored in place (same key, so nothing in the DB moves).
4. ~~**Per-family penalty in `recompute-hotness.mjs`**~~ **MEASURED 09-16, not needed.** `.local-scratch/deals-rendered-families.mjs` on the live order that /deals actually renders (interleave → `spreadFamilies`, which production applies): first 24 cards **0 adjacent, 0 within-3** same-family rows; first 48 → 1 adjacent, 2 within-3; first 72 → 0 and 0. The first screen a phone shows is clean — three Pampers appear at slots 5, 10 and 14, about one per screenful. Revisit only if the rendered order regresses; penalising hotScore would cost ranking quality for a problem the renderer already solved.
5. **Kritikos provenance question** (Step 0 item 3), read-only investigation first.
6. ~~**AdminPanel emoji**~~ **DONE 09-16, row 29.** Its five `confirm()` calls are deliberate and stay.
7. ~~**alert()/confirm() → inline confirmations**~~ **DONE 09-16, row 28.** The public surfaces are clean; `AdminPanel.js` still has five `confirm()` calls and its emoji (item 6).
8. After decision #1 (consent-free measurement): build 3c. (Decision #2 is done — row 23.)

**Step 2 — blocked on the owner (do not start; ask if unclear):** the seven decisions in «⛔ Owner decisions» below (#7 added 09-16: switch on the server profile). Decision #1 (consent-free measurement) unblocks the most: the events from row 19 fire only for visitors who accepted cookies. Decision #2 now has its data: 7,458 ΜΟΝΟ offers carry `baseline_price` / `implied_percent` (6,768 below shelf); a card line «κανονικά ~Y€» is a render-only change in `src/components/DiscountCard.js`. **AB's ΜΟΝΟ offers all sit exactly at shelf price** (227 baselines, none below): at AB the saving is the mechanic (1+1 etc.), so a card line must stay silent there by construction (`implied_percent <= 0`).

**How we work here (conventions that bit us; all in CLAUDE.md, repeated for non-Claude tools):** components are `.js`, libs/actions `.ts`, no TSX migration, vanilla CSS in `src/app/globals.css`. **No emoji in the product UI (owner, 09-16)** — use `src/components/Icons.js` for interface glyphs and `src/components/CategoryIcon.js` for departments; both are stroke SVGs that inherit `currentColor`. The admin panel and the pipeline-health log lines still use emoji on purpose (internal tools). Every DB-touching `.mjs` script loads dotenv BEFORE importing prisma. `Discount.source` isolation and soft-delete only. Never auto-create Products from a matcher verdict. Groq and ab.gr are IP-blocked from the dev PC → pipeline verification is `gh workflow run scrape-chains.yml -f chain=<job> -f limit=N` then `gh run view <id> --log`. Port 5432 is unreachable from the dev PC → DDL over the pooled 6543 URL with `ADD COLUMN IF NOT EXISTS` from a `.local-scratch/*.mjs` script (09-16 examples: `add-click-event-context.mjs`, `add-baseline-columns.mjs`, `add-cheapest-flag.mjs`), mirror it in `prisma/schema.prisma` with matching column/index names, then `npx prisma generate` and delete `.next`. Before every commit: `npm run test:run` and `npm run build`. `npx tsc --noEmit` shows **18 pre-existing errors, all in older test files** — compare the count, do not chase them. Verify UI at 390px (a local `npx next start -p 3100` + Playwright works; production after the Vercel deploy). Measure before assuming — three planned details changed on 09-16 once measured (median → latest shelf price, +10 → +6, a partial index → a plain unique index). On Windows the Bash tool breaks on backticks inside heredocs: write multi-line edits to a scratch file and run it. **Update this START HERE section in the same step as each finished item.**

### Status in one table (2026-09-15, end of day; 09-16 changes are in the table below it)
...
| Comparison chip / positive verdict | **4,976** (31%, was 4,287 this morning) / 3,664 (23%) |
| Offers showing a «Κανονική τιμή» shelf row | **1,898** (was 83 this morning) |
| Products with GTIN | 19,687 / 64,811 (30%) — **unchanged since 09-03** because T11 P1 was never pushed (fixed today; first real runs Sunday 09-20) |
| Mappings with provenance (`matched_via`) | **~80%** stamped tonight (was 2%); kritikos and bazaar nulls convert on their next runs |
...
| Tests / build / CI | 304 green / green / `ci.yml` green |

### Shipped 2026-09-16 (Step 1 items 1–5; all on `origin/main`)

| # | Commit | What | How to verify |
|---|---|---|---|
| 18 | `58928c5` | **W3b homepage.** New first rail **«Τα βασικά της εβδομάδας»**: 10 staples (γάλα, ελληνικός καφές, ελαιόλαδο, φέτα, απορρυπαντικό ρούχων, χαρτί υγείας, ζυμαρικά, αναψυκτικό κόλα, μπύρα, πάνες Νο4), each the cheapest active offer per €/λίτρο-€/κιλό-€/μεζούρα-€/τεμ. across ALL chains, only when 2+ chains have one. Rules in `src/lib/weekly-essentials.ts` are department-scoped with exclusions measured on the live set (soap «Μέλι & Γάλα», chocolate under "cola", feta pies, alcohol-free beer, a Latin-M «Mαλακτικό»); diapers compare inside size 4. Cached action `getWeeklyEssentials` (5 min, `deals:default`). Cards show the staple as an eyebrow. Category grid = 8 departments in shopping order + «Περισσότερες»; chain tiles = one horizontal strip, no dead «Σύντομα» tiles. The newsletter was already in the footer | Production `/` at 390px: rail at the top with prices in the first screen; 10 `card-eyebrow` elements in the HTML |
| 19 | `c885e31` | **W3d events** (all consent-gated). `click_events` gained `position`, `page`, `query`, `resultCount`, `dedupKey` (unique) + index `(sessionId, createdAt)`; columns are camelCase like the rest of that table. New `trackImpressions` action + `src/lib/impressions.js` (≥50% visible for 1 s, batched every 5 s / at 25 / on pagehide, 200 per browser session). Cards carry `list` + `position`; home rails are `home:essentials`, `home:foryou`, `home:top`, `home:ending`, `home:favorites`. Now firing: `search` (query + result count), `filter` (/deals reloads, in-store filters, onboarding departments), `store_select`, `favorite`/`unfavorite`, `list_remove`. `outbound_click` has no call site: the only external link is the leaflet, already `leaflet_click` | Local production build with a marked test session: impressions for home:essentials / home:top / search with slots and dedup keys; `search` «γάλα» resultCount 60 (test rows deleted). Query: `select "eventType", page, count(*) from click_events where "createdAt" > now() - interval '1 day' group by 1,2` |
| 20 | `9de6ae0` | **W2a data side.** `discounts.baseline_price`, `baseline_at`, `implied_percent` for ΜΟΝΟ offers, written nightly by `recompute-price-verdicts.mjs` via `src/lib/baseline-price.ts`: the offer's own chain's **latest** `normal` snapshot while its catalog feed is alive (same semantics as the comparison sheet's shelf rows), dated by that feed run; shelf > 3× offer dropped. **The detail view's «Κανονική τιμή» switched from a 90-day median to the same number** (`get-price-history.ts`). Why: snapshots are written only on change, so the median missed 2,495 offers with stable prices and was >5% off for 480 of 4,981. Not rendered on cards (decision #2) | Ran live: 7,458 baselines, 6,768 below shelf, 690 not below, 0 verdicts changed. Per chain with baseline: mymarket 2,083, kritikos 1,790, masoutis 1,710, sklavenitis 1,636, ab 227 (all at shelf), lidl 12 |
| 21 | `71023bf` | **W3a cheapest in cluster.** `discounts.is_cheapest_in_cluster`: no row on the offer's comparison sheet (rival offers + rival shelf prices) is cheaper, and there is at least one. `comparisonRivals()` in `src/lib/comparison-count.ts` now returns the sheet's rows with prices and the chip count is derived from it. `computeHotScore` adds **+6** (`CHEAPEST_BOOST`; plan said +10 — measured: +5 already puts 27 flagged rows in the top 30, +6 turns the homepage rail into verified-cheapest staples across five chains). Workflow: comparison counts now run **before** hotScore | Ran live: 4,976 offers with a comparison (unchanged), 2,917 flagged, 3,258 hot scores updated. Preview scripts: `.local-scratch/top30-dump.mjs`, `.local-scratch/top-rail-preview.mjs` (`BOOST=n` to try another size) |
| 22 | `5cd35f3` | **W4a anonymous server profile — LIVE since 2026-09-16 (flag on in Vercel Production).** New `profiles` table (random device id, declared stores/departments, favorites, list lines as ids + quantity, top-60 interest weights, text size, `user_id` for a future login merge). `src/lib/profile-snapshot.js` bounds the snapshot, `src/lib/validations/profile.ts` refuses anything larger, `src/actions/sync-profile.ts` has `syncProfile` (flag + Zod + rate limit + upsert) and `deleteProfile` (always allowed), `src/lib/profile-sync.js` + `src/components/ProfileSync.js` sync 3 s after a change and on page hide, only after consent, only when changed; rejecting consent erases. Write-only: no read path. Privacy pages switch wording with the same flag. **Why off:** the live privacy policy promises preferences stay on the device (owner decision #7) | Local build with `NEXT_PUBLIC_PROFILE_SYNC=1`: row appeared with [lidl, ab] / [Κάβα], an add-to-list made items = 1, rejecting consent deleted the row and the local id. Flag-off build: /cookies still says preferences are not sent |
| 23 | `bd3ff86` | **«κανονικά ~Y€» on cards — owner decision #2 approved.** A quiet grey note under the price of a ΜΟΝΟ offer, never struck through (that styling would read as a price the chain published). Shown only between **10% and 50%** below the same chain's shelf price. Two honesty guards added after reviewing real cards: (a) **standing price** — if that chain charged about today's price 45+ days ago, or an older ΜΟΝΟ price undercuts the shelf figure by >15%, no claim is made (My Market's Softex Pro+ sat at ~2.80 from 24/06 to 03/09 against a 5.60 shelf row; Kritikos' Softex Silk showed one 8.50 row from 13/06 while charging 4.68 → 3.83 since); (b) the **50% cap**, because above it the number is usually our data being wrong (a 17.98 misread a week after 7.19 at the same shop; Fairy Platinum 19τεμ and 30τεμ sharing ONE product record). The detail view's «Κανονική τιμή» applies the same standing-price guard, so card and detail never disagree | Live recompute: **2,746 offers hold a baseline, 2,077 qualify to show the note** (was 7,458 before the guards). On /deals at 390px: «κανονικά ~6.77€» under Nescafe Gold at 5.08€, one line, not struck |
| 24 | `33b04bc` | **W4b step 1 — preferred stores became a BOOST, not a filter, in «Για σένα».** The rail used to ask the server for `supermarket in (my stores)`, which hid a cheaper price at a chain the user had not ticked — the opposite of the promise. New pure lib `src/lib/personal-feed.ts` (`rankPersonalFeed`, 18 tests): declared category +10, learned category/brand from the on-device profile, **preferred store +4**, cheapest-in-cluster +2, stable-sorted so hotScore breaks ties. Plus three bounded slots/caps measured against live data: **1 «cheaper elsewhere» slot per 10** (a cheapest-in-cluster row at a chain the user did NOT tick — a reserved slot, not a bigger score, because `is_cheapest_in_cluster` tracks mapping coverage, not reality: mymarket 813 rows, masoutis 421, ab 22, **lidl 0**, so scoring it above the store boost emptied an ΑΒ+Lidl rail of ΑΒ and Lidl), **1 exploration slot per 10** (a department outside the user's declared+learned ones, from the cached hot list, minus what the rails below already show), **max 2 per brand family**, and **max 3 cards per chain the user did not tick** (own chains uncapped — with no stores ticked the live rail came back 13/14 Μασούτης). `HomeClient` now fetches three pools (all chains in my departments / my chains in my departments / plain hot) and ranks them client-side | Local at 390px with ΑΒ+Lidl and dairy+cleaning ticked: 14 cards, 12 from ΑΒ/Lidl, slot 5 = Μασούτης Palmolive 1.21€ («Χαμηλότερη τιμή», verified cheapest of its cluster: mymarket 1.30€), slot 10 = a department outside the two. `.local-scratch/personal-feed-compare.mjs` prints the same for three personas |
| 25 | `fb74195` | **Emoji out of the product UI, small stroke icons in — owner call, 09-16 («it screams so much AI, make it professional»).** `CategoryIcon` went from 17 emoji to 17 hand-drawn stroke glyphs (apple, fish, gable carton, wedge, bowl, tin, loaf, snowflake, basket, cup, bar, glass, pump bottle, teat bottle, trigger spray, paw, parcel — deliberately unlike each other, which is why the emoji were there: an older line set had fruit and personal care both as droplets). `Icons.js` gained Bell, Wallet, Box, Store, Book, Info, Warning. Replaced everywhere they appeared: card verdict pills (🔥/✅ → flame/tick glyph + text), «Τιμές σε N καταστήματα» (⚖️ → storefront), «✨ Για σένα» → «Για σένα», sort chips, cookie banner, footer newsletter + its three status lines, alerts page and its criteria chips, preferences sheet (👋/⚙️/✓), offer detail pack note and add button, shopping list savings hint, supermarket view tabs + aisle headings + leaflet link, category browser art, search empty thumbs, 404/500 pages, legal «needs a lawyer» marker, the shared list text (🏷️/💰 headers → plain), the welcome email. Aisle metadata now carries `icon` (a CategoryIcon key) instead of `emoji` | At 390px on /, /deals, /supermarket/masoutis, /alerts, /cookies, /offer/…: **zero emoji in the rendered text**, no page errors, no horizontal overflow. `python .local-scratch/emoji_scan.py` lists only `AdminPanel.js`, `lib/pipeline-health.ts` and arrows/→ in copy |
| 26 | `dfbb3f8` | **Pack-size guard: two different stated piece counts are never each other's price rival** (Step 1b item 2a). `samePack` in `src/lib/packaging.ts` only knew multibuy («9+3») and multiply («6x330ml») patterns — a bare «30τεμ.» was deliberately ignored, which is how Fairy Platinum 19τεμ and 30τεμ, sharing one `Product`, rendered as each other's rival at 8.99€ vs 17.98€. New `statedPieces()` reads «30τεμ.», «12 ρολά», «4x12τεμ» (= 48) and **only fires when BOTH names state a count**, so a chain that omits it still compares. Everything downstream inherits the guard: the sheet (`get-price-comparison`), the chip (`comparison-count`), `get-cheaper-alternatives`, `resolve-pending-matches` (so the merge stops happening at the source) and `audit-mappings` | Measured on the live set: **98 of 6,924 cross-chain rival pairs dropped (1.4%)**, all genuine mismatches — Babylino 50τεμ vs 46τεμ, Pampers No3 60τεμ vs 48τεμ, Septona Midi No3 vs Maxi No4. 74 offers lose a rival, 35 lose their last one (chip disappears, correctly). `recompute-comparison-counts.mjs` re-run: 883 rows updated, 4,895 offers with a comparison, 2,880 cheapest-in-cluster. `audit-mappings.mjs` now lists **147 pack-only mismatches** for the Review Queue |
| 27 | `557add2` | **Product images shrunk: 800px WebP, at the mirror and in the bucket** (Step 1b item 3b). The Vercel optimizer has been off since June, so a phone downloads whatever the chain published — Κρητικός 1080px JPEGs (some objects **7-8 MB each**), My Market 1000px PNGs, about **5-8 MB for one screen of 48 cards** rendered at ~170px. New `src/scripts/lib/shrink-image.mjs` re-encodes to WebP at 800px and returns the ORIGINAL when that would not be smaller, when the file is animated, when it is not an image, or when sharp throws. `mirror-images.mjs` runs every fresh download through it (`result.shrunk` in the report), and `src/scripts/shrink-mirrored-images.mjs` does the same for what is already stored — read and write over the SIGNED S3 endpoint, same key, so no URL changes and no DB migration. Dry run unless `APPLY=1`; `PREFIX` / `LIMIT` / `MIN_BYTES` / `SAMPLE` / `CONCURRENCY`. Workflow button: **`gh workflow run scrape-chains.yml -f chain=shrink-images -f apply=true`** (optional `-f prefix=kritikos/`) | Ten live Κρητικός objects, applied and checked: **71.3 MB → 0.2 MB**, all still 800×800, served as `image/webp` over the public URL and decoded by a real browser. **Full backfill 09-16 (`gh run 35111417083`, ~1h40): 24,658 objects rewritten, 2,661 left alone, 0 failed, 2,968 MB → 826 MB (72% smaller)** — mymarket 1,451→449 MB, kritikos 1,067→198 MB, lidl 172→17 MB, ab 57→7 MB; bucket total **3,304 MB → 1,091 MB**. Live image weight per page (390px, scrolled): **/supermarket/kritikos 2.84 → 0.81 MB**, /deals 1.59 → 0.67 MB, / 0.95 → 0.86 MB, /supermarket/mymarket 2.86 → 1.78 MB (its packshots stay the heaviest at 800px). Re-run the job after a big new scrape |
| 28 | `6262211` | **Inline confirmations instead of `alert()` / `confirm()`** (Step 1b item 7). A system dialog speaks in the browser's voice, names the website rather than the action, and on a phone covers the thing it is asking about. Emptying the shopping list now arms its own button («Σίγουρα;» + the item count + «Ακύρωση») and disarms after 5 s; deleting a price alert does the same inside its row; copying the list or an offer link answers with a short note next to the button. New CSS: `.btn-danger`, `.drawer-note`, `.sheet-copied` | Playwright at 390px: add two items → open the list → «Άδειασμα» arms and explains → second tap empties it, with **zero browser dialogs** in the run (`.local-scratch/confirm-flow.mjs`). `AdminPanel.js` keeps its five `confirm()` calls — internal, still on the list with its emoji |
| 29 | `9b33859` | **Admin panel on the same icon set** (Step 1b item 6). Tabs get glyphs instead of 📋📚📖🧐🩺📊📧➕ (tag, grid, book, search, pulse, chart, mail, plus), the pipeline-health column shows a tick or a warning triangle instead of ✅/⚠️, row actions use the same bin and star as the public cards, and the reload buttons, spinners, search placeholders and toasts drop their decoration. `Icons.js` gained Lock, Download, Chart, Mail, Pulse. **The five `confirm()` calls stay on purpose** — one of them empties an entire review queue, and a dialog that cannot be mis-tapped is the right guard there | `python .local-scratch/emoji_scan.py` reports **no emoji left in `src/components` or `src/app`** (arrows ← → ↗ in copy are typography, not emoji); lint 0 errors |
| 30 | `390a1fa` | **The «Για σένα» pools are cached; only the ranking is per-device.** The rail fired three uncached queries per visit (four with preferred stores). The pools are not personal — «hottest offers in these departments», optionally narrowed to some chains — so `getPersonalPool` wraps them in `unstable_cache` (5 min, tag `deals:default`), keyed on the SORTED department and chain sets so two orderings share one entry. `lib/personal-feed` still ranks in the browser, so the rail stays personal | Same rail at 390px before and after: 14 cards, saving slot at 5, exploration slot at 10. Production smoke after deploy: four rails, 143 SVG icons none zero-sized, no emoji, no page errors, and the list's «Άδειασμα» arms inline with zero browser dialogs |
| 31 | `a7d314b` | **Two things a CI resolver run caught** (`gh run 35113586372`, the smoke test for row 26). (a) `samePack` refused «Fanta …330ml **6τεμ**» against «FANTA …KOYTI **6*330ML**» — one six-pack, two spellings — and «…**90Χ60cm** 15τεμ» against «…**90x60cm** 15 Τεμάχια», where `packCount` read the Latin-x dimension as a 90-pack and the Greek-Χ one as a single, so it disagreed with itself over a printed SIZE. A count that both names state is stronger evidence than the multiplier and now decides; `packCount` still rules when neither spells one out, and a name that states nothing is never blocked by one that does. (b) The run **aborted at item 24 of 50** on a Groq `400 json_validate_failed` — the model spent its output budget thinking and never closed the JSON for one product name. That is a per-item miss, not a broken config: the row now stays pending and the run continues. `max_completion_tokens` 1024 → 1536 (only emitted tokens count against the daily cap) | Re-ran `gh workflow run scrape-chains.yml -f chain=mymarket-resolver-only -f limit=30` → **success, 2 resolved, 0 errors, no abort**. Live impact of (a): 76 cross-chain rival pairs drop instead of row 26's 98 — the 22-pair difference is pairs it was wrong to separate |
| 32 | `79b29f5` | **«Τα καταστήματά μου» stops filtering `/deals` and starts leading it** (the half of W4b that was left). The browse page answered a shopper who had ticked chains with `where supermarket in (mine)`. Measured on the live set: an **ΑΒ+Lidl shopper saw 471 of 15,907 offers (3%)** under the title «Όλες οι προσφορές», and **2,951 cheapest-in-cluster rows were hidden from them** — the cheaper-elsewhere prices this app exists to surface. Row 24 fixed the same bug on the homepage rail by re-ranking in the browser; a paginated list cannot, because ΑΒ is 2% of live offers and its rows are simply not in the first 24 by hotScore. New `src/actions/get-blended-deals.ts` runs **two hot-ordered streams** (their chains / everyone else) with independent cursors and `blendStreams` (in `src/lib/personal-feed.ts`) alternates them one-for-one, starting with theirs. The streams partition the live set, so pages never overlap or repeat and `total` stays the count of everything. **Only the merchandising sort blends** — a blended «Τιμή: χαμηλή → υψηλή» would not be sorted at all, so an explicit sort now gets a plain all-chain query in the order asked for (it used to be silently limited to their chains); explicit chips still filter exactly. The banner stopped claiming a filter («Φιλτράρεται από τα αγαπημένα σου καταστήματα») and now says what is true, with the old behaviour one tap away («Μόνο τα δικά μου» → real, clearable filter chips). Cached 5 min per chain-set, tag `deals:default`, shared by every shopper with the same chains | Playwright at 390px with ΑΒ+Lidl ticked (`.local-scratch/deals-blend-flow.mjs`, `deals-blend-modes.mjs`): page 1 = **ΑΒ 8 + Lidl 4 of 23 cards**, the rest across the market, count line **«από 15.907»** (was 471); 3 pages, 70 cards, **0 repeated cards**; «Μόνο τα δικά μου» → 24 cards, all ΑΒ/Lidl, two clearable chips. No prefs → byte-identical to before. `?sort=price_asc` → all chains and genuinely ascending (0.22 → 0.22 → 0.25 → …). A department + prefs → blended inside it (Lidl 11 + ΑΒ 1 of 23, count 2.464). Simulated over the same data (`.local-scratch/deals-personal-sim.mjs`): cheapest-in-cluster rows on page 1 go **1/24 → 10/24, all ten at chains they did not tick** |
| 33 | `cb08562` | **Daily watchdog on the hosts that serve product photos** (owner decision on the image domain: *watch now, custom domain before promoting the site*). `src/scripts/check-image-hosts.mjs` samples live offer images per host — evenly through the list, not the first N, so one bad upload batch cannot masquerade as the whole host — fetches each and fails the run below 90% reachable. New `image-hosts` job in `scrape-chains.yml`, 08:00 UTC beside `pipeline-health`, plus `-f chain=image-hosts` on demand. **Only a host serving 25+ images can fail the run**; a leftover hotlink is reported as a straggler to re-mirror, because an alarm that cries wolf every morning is ignored by the time it matters. It also reports whether each host sends `Cache-Control` | Live: **r2.dev 11,769 images, 100% reachable, p50 1.6 s, and `Cache-Control` on 0 of them**; `s1.sklavenitis.gr` 3,495 images, 100%, `Cache-Control` on all. First run found **one My Market offer still hotlinking `cdn.mymarket.gr`, now 502** — its image was cleared so the card shows the placeholder. **Correction to the note in row 27:** the `r2.dev` DNS blackhole is NOT reproducible — it resolves and serves normally; what was seen earlier was this dev machine (its traffic exits via a Seattle edge, i.e. a VPN). The case for a custom domain is now Cloudflare's own production guidance plus the missing `Cache-Control`, not a resolver problem |
| 34 | `4300e31` | **Anonymous measurement — owner decision #1 answered: yes** (`NEXT_PUBLIC_ANON_ANALYTICS`, ships OFF; set it to `1` in Vercel Production to switch on). Five months of consent-gated tracking produced **917 events, 28 distinct visitors in 30 days**, so every ranking number (store boost +4, cheapest +2, the 50/50 blend, the reserved slots) was chosen by reasoning and simulation and never checked against a shopper. New `src/lib/analytics-mode.js` is the single decision point: **`identified`** (consented → events carry the persistent `sid`), **`anonymous`** (no consent, flag on → events carry NO identifier), **`off`** (no consent, flag off → the old hard no-op). The cookie rule is about storing or reading on the device, so the anonymous mode does neither — no id, no cookie, no localStorage, **no sessionStorage either: the impression counter moved into memory and caps per page load instead of per tab** — and the server stores no user agent and no IP (the IP is read for rate limiting and dropped with the request). **`userAgent` is no longer written at all**: nothing ever read it in five months while it was the most identifying column in the table. Rows written before 09-17 still hold theirs — clearing them is a one-line UPDATE, owner's call. Anonymous impressions leave `dedupKey` null (Postgres allows many nulls in a unique index) and the duplicate guard moves to the client queue, which already refuses a card it queued this page load. New column **`click_events.context` ('mine' \| 'other')** = was this card's chain one the shopper ticked: it describes the SLOT, not the person (thousands share each value), and it is the only way to tell whether the store boost of rows 24/32 earns its place once events carry no id. Rate limits 60→240 events/min and 30→120 impression-batches/min per IP, because events now come from everyone and Greek mobile carriers put many shoppers behind one address. `/cookies`, `/aporrito` and a transparency line in the banner («ακόμη κι αν πατήσεις Απόρριψη…») all switch on the same flag | Playwright at 390px, flag on, banner **never answered**: **25 rows written** where the old build wrote 0 — 22 impressions, 1 deal_click, 1 filter, 1 page_view — with **0 session ids, 0 user agents, 0 dedup keys**, slot context on 23 of 25 (`mine:12 / other:11`, the blend visible in the data) and 22 of 22 impressions distinct. Re-run with consent accepted: 24 rows, all carrying `sessionId`, 21 dedup keys, still 0 user agents. 8 new unit tests pin the mode matrix (incl. «an accept is honoured even with the flag off») |
| 35 | `5c16954` | **Tab-scoped visit id + the banner's own answer** (owner, 09-17: "add it now, I'll talk to a lawyer later"). **Both ship OFF.** (a) `NEXT_PUBLIC_VISIT_ID=1` → `src/lib/visit-id.js` mints `v-<uuid>` in **sessionStorage**, so it dies with the tab and can never link two visits, two days or two devices. It buys funnels (search → scroll → tap, where a visit gives up), which aggregate counting cannot reconstruct. **Unlike the rest of the anonymous layer this IS storage on the device**, so it is not exempt by construction — it rests on the audience-measurement reasoning EU regulators apply to first-party analytics, which holds only while the purpose stays *measurement*. Hence its OWN flag: counsel can refuse exactly this and cost none of row 34. The `v-` prefix is deliberate — an anonymous visit is distinguishable from a consented session at a glance, and every row it produced is removable with one `WHERE "sessionId" LIKE 'v-%'`, because this shipped BEFORE the legal advice. `/cookies` lists `pp-visit` in its storage inventory and the banner's transparency line stops claiming nothing is stored, both on the same flag (an undisclosed key is what actually gets a site fined). Withdrawing consent erases it. (b) New `consent_choice` event, **always sent with no id in either direction**: refusing currently leaves no trace at all, so nobody can say whether consented profiles reach 5% of shoppers or 60% — and that number is the ceiling on every consented feature and the only honest route to per-person profiles. **What was NOT built, deliberately:** durable cross-visit profiling of non-consenting visitors. That is not a grey area, it is precisely what the banner exists for, and a profile without a lawful basis is a diligence liability rather than an asset | **422 tests green** (8 new on the visit id: flag-off writes nothing at all, a consented visitor gets none, blocked storage stays anonymous instead of throwing, reject forgets it), lint 0 errors, `tsc` at the 18 pre-existing. **NOT yet verified in a browser** — the session's tool permissions refused to start a dev server with these flags set, so the wiring (a `v-` row actually landing, `consent_choice` firing from the banner) is unit-tested only. Both flags are OFF, so production is unaffected until someone turns them on. **Do that browser check before flipping `NEXT_PUBLIC_VISIT_ID`** |
| 36 | `fb45bf8` | **The browser check of row 35, and the bug it caught.** `NEXT_PUBLIC_ANON_ANALYTICS=1` went live in Vercel on 09-17 (production `/cookies` now serves the anonymous-measurement heading; `pp-visit` correctly absent). The visit id then verified at 390px with the banner never answered. **The bug:** `startProfileSync` declared `timer` AFTER the consent listener closing over it and returned early when `NEXT_PUBLIC_PROFILE_SYNC` was off, so a later «Αποδοχή» hit `schedule()` with `timer` in its temporal dead zone and threw inside the consent callback. Production has that flag on, so it never fired live — but turning it off would have made **every accept throw**. Declared before the listener and the accepted branch now guards on the flag. The visit id is also dropped on either choice now (a refusal should leave none; an acceptance replaces it with the real `sid`) | 25 rows from one `v-adc98a35…` id, **0 real sids, 0 user agents**, 21 impressions carrying their dedup key again. `consent_choice` rows land from both buttons with **no id and no user agent**. After the fix: accept → no page errors, visit id cleared; reject → visit id cleared, no sid minted |
| 37 | `845eca1` | **Accept-first banner layout, behind `NEXT_PUBLIC_CONSENT_ACCEPT_FIRST`** (owner's explicit decision, 09-18; ships OFF). Set it and «Αποδοχή» stands alone on the first layer with «Περισσότερα» beneath; expanding reveals the two cookie categories and «Απόρριψη μη απαραίτητων». **Read this before changing it either way:** EU regulators treat a reject costing more clicks than accept as a dark pattern, and the finding is not that it risks a fine but that the consent collected is **invalid** — so the `profiles` rows resting on it have no lawful basis, which bites hardest at sale, where a buyer's counsel reads that table as a deletion obligation rather than an asset. The owner was told this three times and chose it; it is a flag so counsel can reverse it in one switch. **Held in BOTH layouts and not negotiable:** declining is one real click once visible (never a toggle menu or a confirm step), the banner stays re-openable from the footer, and the anonymous-layer disclosure stays on the FIRST layer because it describes what happens to someone who never expands. The accept-first first layer is deliberately short, the long copy moved into the expansion | At 390px: accept-first layer 1 = `[Αποδοχή, Περισσότερα]` at **295px tall** (448px before the copy moved; default layout is 397px), expanded = `[Αποδοχή, Απόρριψη μη απαραίτητων]` at 575px; declining resolves the banner in one click and stores `rejected`. Flag off → `[Απόρριψη, Αποδοχή]`, unchanged. 422 tests green, lint 0 errors, `tsc` at the 18 pre-existing |
| 38 | `3ce06fc` | **AB offers were at zero for six days — fixed** (09-23). `ab-offers` failed every night from **09-17** (341 offers on 09-16, then 0; `pipeline-health` went red each afternoon because of it). The log line was `page 1/117 — unique: 0/1169`: hash accepted, pagination parsed, no products. Cause, measured in CI with `src/scripts/probe-ab-offers-shape.mjs` (manual job `ab-offers-probe`; ab.gr blocks the dev PC): **AB now removes items from each PROMOTION_SEARCH page after paging** — about half the listing is hidden from a logged-out visitor — so at the adapter's 10 per page most pages were empty, and the adapter treated the first empty page as the end. `includePotentialActivatableOffers` made no difference (my first suspect; wrong). Size 50 → 24 pages, none empty, 599 unique of 1,170, no duplicates; **size 100 returns nothing** (AB's ceiling is 50). Fix: `PAGE_SIZE` 10 → 50, and keep walking until `totalPages` (an empty page only ends the walk when pagination is missing). The run now prints `visible: N of M listed promotions (%)` — watch it; a further fall shows there first. The guards held all week: nothing written or deactivated on the failed nights, and public queries hide expired offers (`validUntil > now`), so shoppers saw a thinning AB (144 visible), not wrong prices | Live run `35841423702`: visible 599/1170 (51%) → **275 price-affecting**, 275 matched, 30 price changes, 189 deactivated (ended offers finally closed), health ✅. AB offers visible to shoppers **144 → 275**. **Open:** 275 vs 341 on 09-16 — normal weekly churn or offers AB now hides from logged-out visitors; can't tell yet. 432 tests green |
| 39 | `2e19e45` | **The image-domain move, prepared** (09-23; the DNS part still needs the owner). `src/scripts/rewrite-image-host.mjs` (+ pure helpers in `src/scripts/lib/image-host.mjs`, 10 tests) swaps the base in front of each object key. **Scope is 59,557 rows in THREE tables** — `discounts` 27,571, `products` 31,096, **`pending_matches` 890** (missing from the earlier plan; the Review Queue would have kept the old host). Dry run unless `APPLY=1`; batched; re-runnable. **Refuses to write** unless a random sample is served by the new host as the same file (200, `image/*`, identical byte length). Switching `R2_PUBLIC_URL` is safe at any point: `isMirroredUrl()` and the coverage metric treat both `*.r2.dev` and `R2_PUBLIC_URL` as mirrored, so there is no re-upload storm. **DNS inventory** (queried `ns1.vercel-dns.com` directly): apex A, `www` A, a wildcard `*` A, CAA — and **no MX, no SPF, no DKIM, no Resend records**, so moving the zone to Cloudflare cannot break email (none is configured). Also corrected the `next.config.mjs` note: the r2.dev "Greek resolver blackhole" was this machine on a VPN | Dry run against the domain as it is today (Vercel wildcard → 404 on every key): `new host served 0/9` → aborted, nothing written. Owner steps are in «Image domain» in START HERE |
| 40 | — | **Pickup check, 09-23 (read-only).** (a) **Sunday 09-20 canonical runs happened and worked** (Step 0 item 4): products with barcode **19,687 → 25,247**; wolt runs for ab 5,693 / masoutis 6,288 / mymarket 6,893. (b) **Σκλαβενίτης "167" is not a failure**: `scrapedItems` counts barcode-backed items only; the run saw 6,869 items and Wolt exposes a GTIN on **2.8%** of them (was ~5% on 09-03) — the documented loss in the workflow comment. (c) **Traffic is effectively zero**: since 09-19, **19 events from 1 id**; all 10 `consent_choice` rows are dated 09-17 and are our own Playwright clicks. The "wait a week, tune the ranker from clicks" plan has no data to run on — the constraint is visitors, not measurement. `/sitemap.xml` is live (15,061 URLs). Production flags: `ANON_ANALYTICS` on; `VISIT_ID` and `CONSENT_ACCEPT_FIRST` off (production `/cookies` has no `pp-visit`) | — |

### Shipped 2026-09-15 (all on `origin/main`, deploy verified live)
...
| 7 | `878ec50` | **Sentry was dead**: `sentry.*.config.js` existed but nothing imported them (Next 15+/Sentry v8+ need `src/instrumentation.ts` + `src/instrumentation-client.ts`). Added. Every `captureException` in `src/actions` and the watchdog had been a no-op | Owner: confirm `SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN` exist in Vercel env; throw a test error |
...
| 14 | `066c6de` | **W1c + W1e (Opus subagent, reviewed)**: `matchItem` re-verifies and stamps `matched_via='barcode'` on every mapping hit that carries a GTIN; `ingestCatalog` stamps in batch, gains the missing **volume guard** (reference = last representative run, half-of-peak) and `refreshFields`; **Kritikos canonical now goes through `ingestCatalog`** (first ever IngestRun + SKU mappings); `backfill-matched-via.mjs`; `EXPECTED_FEEDS` covers catalog/baseline/wolt feeds with a `graceUntil` for the five that cannot have run before Sunday; **`ci.yml`** runs test + lint + build on every push | Backfill **APPLIED**: 48,762 of 61,060 null mappings stamped (`catalog` 36,422 / `cache` 12,340). Remaining nulls: kritikos 5,918 (→ `barcode` via the dispatched `kritikos-canonical` run `34994550427` + tonight's offers run), bazaar 6,342 (→ next runs). Re-run `audit-comparison-truth.mjs` after Tuesday's nightly |
...
**Kritikos catalog walk `34994550427`:** finished green on 09-15 (8,061 items, healthy IngestRun), but only 44 of 5,918 null mappings became `barcode` — open question in Step 0 item 3.

### What is actually wrong — ranked by what a shopper feels (the audit; status tags added end of day)

**A. [OPEN — owner decision #1] The first screen is a legal dialog, then a second dialog.** On first visit the consent sheet covers ~35% of every page (screenshot-verified), and the onboarding sheet opens 700 ms after it is answered. Two modals before a single price. For this audience that is the whole first impression.

**B. [FIXED 09-15, row 13] "Κορυφαίες προσφορές" leads with floor wipes and three chewing gums.** Live top-10 by `hotScore`: Rispet wipes (−65%), Έλμα gum ×3 (−50% + "1+1"), coconut milk, alcohol-free beer, Coca-Cola ×3, Nivea; 6 of 10 from mymarket. Not clicks (all `click_count` 0) — the formula in `lib/hotness.ts` rewards % and "1+1/δώρο/super" on anything, has no department prior (food staples vs cosmetics), and collapses only exact duplicates, not product families. 32% of all active offers are Προσωπική Φροντίδα (real — chains push cosmetics), so without a prior the feed reads like a pharmacy.

**C. [PARTLY — €/κιλό shipped (row 12); «κανονικά ~X€» waits on decision #2, data side is Step 1.3] For 94% of offers the card gives no reason to believe it is a deal.** No %, no "κανονικά ~X€", no €/kg. Yet **~5,000 of those ΜΟΝΟ offers already have a same-chain shelf price** (`PriceSnapshot kind='normal'`, last 90 days) that the card never uses; the verdict pill covers only 23%. This is the single biggest untapped honest-savings signal in the system.

**D. [OPEN — product call] Validity dates.** The product rule says "always visible"; the data says only AB and Lidl publish them. The UI handles it honestly (no fabricated chip), but «Τελειώνουν σύντομα» is honest only because only AB/Lidl rows ever fall inside 7 days. Accept the limitation and say it plainly in the UI ("ελέγχθηκε σήμερα" prominently, not as a footnote), or get dates from the chains that have them (Kritikos' API and Masoutis' leaflet know their windows).

**E. [MOSTLY FIXED 09-15 — rows 10, 14, 15; hedging of name-matched rows waits on decision #3] Comparison is invisible on 73% of offers and unhedged on the rest.** `PriceComparison` renders nothing (no empty state) when there are no rows. Barcode-proven and name-matched rows look identical. The shelf-price layer («Κανονική τιμή» rows) renders for **83 of ~14,000** offers because `shelf-comparison.ts` demands a snapshot <14 days old while `ingest-catalog` writes one only on price change; the right gate is *feed freshness* (did the chain's catalog run recently?), which restores ~2,000 rows at zero honesty cost. `matched_via` is null on 98% of mappings because stamping only happens on NEW binds, so the T6 "gate on barcode" decision is unmeasurable until a backfill stamps existing rows.

**F. [OPEN — Step 1.2 builds the events; decision #1 unlocks them] Measurement is blind by design.** `lib/track.js` refuses to fire without consent and `session-id.js` will not even mint an id, so 5 months produced 506 events. Only 4 of the 9 declared event types fire anywhere (`search`, `filter`, `store_select`, `favorite`, `outbound_click` have zero call sites). **Search queries are never logged** (Vercel Analytics strips `?q=`). No impressions, no positions, no removals. The profile is `localStorage['pp-interests-v1']` and the feed caches have no user dimension. You cannot learn what users want with this instrument, and the weeks-4–6 distribution experiment would run blind.

**G. [PARTLY — 44 px buttons, bigger type, text-size switch shipped (rows 12, 17); contrast, Greek heading font, iOS zoom, native alert()/confirm() still open] Elderly-unfriendly details, all measured in `globals.css`:** add-to-list button 34 px (30 px in the two-row rail; guideline 44), list qty steppers 24 px, verdict/compare pills 10 px, source tags 9 px on every card, secondary text `--ink-3` at 3.5:1 contrast, placeholders 2.2:1, inputs at 14–15 px (iOS auto-zooms on focus), no text-size control although `data-density="comfortable|roomy"` CSS already exists and is unreachable, DM Serif Display loaded without a Greek subset so every section heading falls back to a system serif, native `alert()`/`confirm()` in the main flow, the gear icon is the only way into preferences.

**H. [OPEN — owner keys, decisions #4/#6] Three dead ends.** `/alerts` is reachable only from emails that cannot be sent; login has zero providers (`auth.ts` builds `[]` — no Google/Apple/Resend env); push cannot register without a session; the newsletter box admits "we'll tell you when it starts".

**I. [MOSTLY FIXED 09-15 — Sentry wired (row 7), CI gate + catalog feeds watched + Kritikos through ingestCatalog (row 14), laptop task limit 6 h; the laptop dependency itself remains (decision #5)] Ops.** Sklavenitis runs on the owner's laptop; the CI `sklavenitis-*` jobs are permanent green **no-ops** (no `PROXY_URL`) — decoy checkmarks. The laptop task runs without R2 env, so Sklavenitis images are hotlinked, not mirrored. Catalog feeds have no freshness/volume alarm: the 09-13 Sklavenitis catalog task was killed (exit 0xC000013A) and its 29-item run was recorded `health_ok=true`. `kritikos-canonical-scraper.mjs` bypasses `ingestCatalog` entirely (no IngestRun, no rails). No test/lint/build workflow runs on push. Two live search queries (`search-deals.ts`, `get-catalog-products.ts`) do leading-wildcard `LIKE` seq scans; the GIN trigram index from `enable-pg-trgm.mjs` cannot match because the query uses `unaccent()` not `f_unaccent()` (the script's own last line says to change that; never done). Cold homepage TTFB 5.7 s; `page.tsx` awaits `isAdminAuthenticated()` which forces `no-store` on the most-visited page.

**J. [NOTED] Stale notes to stop trusting:** `hotness.ts` says the category field is "348 values, ~34% Άλλο" — today Άλλο is **3.7%** (595 rows) across 17 departments. The 09-03 block says "tree clean, everything pushed" — it was not. `EXPECTED_FEEDS` and the workflow disagree about who runs Sklavenitis.

### The plan — five workstreams, in order; each step ships on its own

**W1 — Unstick the machine** (1a–1e DONE 09-15, rows 1–14; open question on Kritikos provenance in Step 0 item 3)
...
**W2 — Honest savings on every card** (2b, 2c DONE 09-15; **2a data side DONE 09-16, row 20** — the stored number is the latest shelf price, not a median; card rendering waits on owner decision #2)
...
**W3 — A feed that looks like a Greek shopper's week** (3a DONE 09-15 + cheapest boost DONE 09-16 at +6, row 21; **3b DONE 09-16, row 18** — alert()/confirm() → inline still open; **3d DONE 09-16, row 19** — not done from 3d: nullable `userId` (no login yet) and first-clicked position on a search; impressions dedup via a DB unique key, other events still use the in-memory limiter; 3c waits on owner decision #1)
...
**W4 — Personalization that survives a device** (≈ 5 days; **4a DONE 09-16 but OFF**, row 22 / decision #7; 4b next after the small items)
...
- 4b `getPersonalFeed(profileId)`: favorites' live hits → cheapest-across-chains within the user's categories → **preferred stores as a boost, not a filter** (DONE 09-16 client-side, row 24 — `lib/personal-feed.ts`) → 2 exploration slots per 20 (DONE) → family collapse (DONE). Still to do: the server-side half, now that `profiles` is live — one cheap uncached query per visitor, global rails stay `unstable_cache`d.
...
**W5 — The vertical "reels" feed** (design only until W3d + W4 exist; W3d exists since 09-16, W4a does not). It is 4b paginated by cursor with **one impression logged per card**, full-bleed image, price + "κανονικά ~Y€" + cheapest-chain line, swipe = next, add-to-list as the single action. Without impressions (3d) and a server profile (4a) it cannot learn, so it comes after them, not before.
...
2. ~~**«κανονικά ~Y€» on cards**~~ **DECIDED 2026-09-16: yes**, on my recommendation — grey note, no percentage on the card, 10–50% band, shipped in row 23. No follow-up needed unless the wording should change.
...
7. ~~**Switch on the anonymous server profile (W4a).**~~ **DECIDED 2026-09-16: yes.** Owner added `NEXT_PUBLIC_PROFILE_SYNC=1` in Vercel → Production (**Config**, not Secret — a Secret is not inlined into the browser bundle, so the flag would stay off) on 2026-09-16. The value is baked in at BUILD time, so it only takes effect on the next deploy. **VERIFIED LIVE 2026-09-16:** /cookies and /aporrito now carry the server-profile wording (the old «δεν αποστέλλονται σε εμάς» line is gone), and an end-to-end run on production created a `profiles` row on consent ([lidl, ab] / [Κάβα]), updated it on an add-to-list, and deleted it on reject. The table is otherwise empty — real rows appear only as consenting visitors arrive.
...
##### T1 follow-up — 2026-09-03: the nightly resolver was starving its last chains

Watching the backfill run (`33648713039`) surfaced an ordering problem the new pacing created. Groq's free tier forces ~13 s between calls, so masoutis' ~1,190-row queue alone needs **4.3 hours** of the job's 350-minute budget. The run worked through AB → sklavenitis → bazaar → mymarket, reached masoutis, and spent the remainder there — **masoutis-leaflet and lidl were never reached**, and would not have been on any later night either, because the step order is fixed. Lidl is exactly the queue that matters most: 131 of its 192 offers are productless.

Each resolver step is now capped at `LIMIT=150` per night. Slower overall drain, but every chain is touched every night instead of one large queue permanently blocking the tail.
...
> **🔴 T6 HAS CHANGED SHAPE — read this before answering the question below (measured 2026-09-03, `npm exec -- node src/scripts/audit-shelf-gating.mjs`).** The shelf-price rows the gating question is *about* are already almost gone, for a reason that has nothing to do with provenance: they render for **83 offers out of 13,991**.
>
> Cause: [ingest-catalog.mjs](src/scripts/lib/ingest-catalog.mjs) writes a shelf snapshot **only when a price changes**, but [shelf-comparison.ts](src/lib/shelf-comparison.ts) discards any snapshot older than **14 days**. A product whose price has been stable for a month has no snapshot inside that window, so it shows nothing — even though the price we hold is exactly right. The module's comment assumes the opposite ("catalogs refresh weekly, so a snapshot older than the cutoff means the chain's feed is unhealthy"), which would only hold if snapshots were written every run rather than on change. The catalog jobs are healthy; 2,787 products have a shelf price recorded in the last 14 days.
>
> Holding every other rule constant, widening only that window: **14d → 85 rows · 30d → 708 · 90d → 2,032 · 180d → 2,032** (plateau). So the gate is suppressing ~**96%** of the shelf prices already collected.
>
> **NOT fixed unilaterally**, because restoring ~2,000 rows is user-visible and lands inside exactly the question below — whether unproven rows should be shown at all. Fixing it first would pre-empt the decision instead of informing it. The honest framing is therefore: *do we want a shelf-price layer at all, and if so how is it gated?* — where the real gate is feed freshness (did this chain's catalog run recently?), not snapshot age, since a stable price stays true until the scraper says otherwise.
...
### What happened (all validated: build ✓, 193/193 tests ✓, Playwright visual pass ✓)
- **P0 — freshness emergencies.** (a) **Masoutis leaflet cron was dead**: the Vercel Thu cron ran the ~3k-item leaflet in-process, but Vercel kills at 300s and the run needs 10–12 min → died mid-run every week, no IngestRun row, no alarm (pipeline-health rode the same dead Vercel cron set). All 3,029 leaflet rows were expiring 07-10. **Refreshed locally (2,900/2,900 ✓, new leaflet period, 1,204 price changes)** and moved masoutis web (daily 03:30 UTC) + leaflet (Thu 03:00 UTC) + pipeline-health (08:00 UTC, curl -f with `CRON_SECRET`) into [scrape-chains.yml](.github/workflows/scrape-chains.yml); `vercel.json` crons emptied — **GH Actions is now the single scheduling truth** (the cron routes remain as manual endpoints). (b) **Sklavenitis: Akamai now 403s this home IP too** (verified — even real Chromium gets Access Denied; worked 06-25; blocks rotate). Offers stale since 06-25, expire ~07-09 → **chain goes offer-less until the block rotates or a proxy appears; accepted** (owner chose free local path over paid proxy). Built the official local path: `\Prosfores\` **Windows Scheduled Tasks** (offers daily 02:30 local, catalog+mirror Sun 04:00) → [scripts/windows/run-sklavenitis.ps1](scripts/windows/run-sklavenitis.ps1) (file logs in `logs/sklavenitis/`, missed-start catch-up, retries). Tasks registered Interactive (S4U needs one elevated re-run of [register-sklavenitis-tasks.ps1](scripts/windows/register-sklavenitis-tasks.ps1)). REQUIRE_PROXY branches are now **quiet skips** — the old fake failed IngestRuns kept admin Υγεία permanently 'warn'; staleness alarming now lives in EXPECTED_FEEDS (sklavenitis window 48h, [pipeline-health.ts](src/lib/pipeline-health.ts)).
- **P1 — honest expiry dates.** Only AB + Lidl publish real end dates; everyone else got ingest's +14d default which the UI showed as a real «Έως DD/MM». New `Discount.datesFromSource` (column applied via pooled ALTER — **port 5432/DIRECT_URL unreachable from this machine**, pooled 6543 works) set by `writeOffer` from `Boolean(item.validUntil)` on every create/update. New [expiry-label.js](src/lib/expiry-label.js) `expiryInfo()` drives BOTH DiscountCard chip + OfferDetails boxes: real → Έως/Λήγει + urgency; default → «Ελέγχθηκε DD/MM» / «Σε ισχύ» from `updatedAt`. SEO gated too (offer metadata + JSON-LD `priceValidUntil`). Lidl active rows backfilled (144); AB self-corrects on its nightly run; admin-created offers flag real when dates typed/leaflet-adopted.
- **P2 — comparison shows shelf prices.** [get-price-comparison.ts](src/actions/get-price-comparison.ts) appends «Κανονική τιμή · DD/MM» rows (latest `kind='normal'` snapshot per chain, ≤14d, new pure [shelf-comparison.ts](src/lib/shelf-comparison.ts) + 6 tests) for chains with no active offer. **Barcode-gated**: snapshots carry no chain-side name for the variant guards, so only the GTIN cluster is safe (mapping-only clusters hold the stale mis-mappings; Groq audit still deferred). ΦΘΗΝΟΤΕΡΑ goes to the absolute cheapest row, shelf or offer (verified live: Coco Pops «offer» AB 4.48€ vs Bazaar shelf 2.99€). **1,152 active offers gained comparison rows** (live spot-check).
- **P3 — catalog prices + search.** Non-offer catalog tiles show last-known shelf price + «τιμή ραφιού DD/MM» (`attachShelfPrices()` in [get-catalog-products.ts](src/actions/get-catalog-products.ts), `CatalogProduct.shelf`; no freshness cutoff by design — the printed date carries the honesty). `/search` gained a deduped «Από τους καταλόγους» section under the deals (offers first). Verified live: nutella = 3 offers + 8 catalog items.
- **P4 — supermarket UI consistency (the owner's "so inconsistent" complaint).** Root cause: hero/listing-head/load-more were one big **inline-style island** with off-token radii (18/14px) + JS hover. Extracted to `.sm-hero*`/`.listing-head`/`.load-more-wrap|btn` classes in [globals.css](src/app/globals.css); radii snapped to `--r-md/--r-lg`; category cards aligned to `.card` language (var(--line)/var(--surface)/−2px hover, brand accent kept); ~130 inline lines removed from [SupermarketClient.js](src/components/SupermarketClient.js). Screenshots (kritikos desktop+360px, masoutis, /catalog, /search, offer page) in `.local-scratch/shots/` — all clean. **Owner has NOT visually signed off yet — click-through after deploy.**

### ⛔ On the owner (for full effect)
1. **Push**: `git push origin main` (5 commits). Until pushed, the new GH jobs don't exist → **Masoutis leaflet has no scheduler again** (next cliff ~07-19) and none of P1–P4 deploys.
2. **Create the GH secret** (permission-gated for the agent): Git Bash → `SECRET=$(grep -m1 '^CRON_SECRET=' .env.local | cut -d= -f2-); printf '%s' "$SECRET" | gh secret set CRON_SECRET`. The pipeline-health job 401s without it. (Note: `.env.local` had a stale duplicate CRON_SECRET shadowing the real one — removed this session; the remaining value authenticates against prod, verified HTTP 200.)
3. **Sklavenitis decision point**: home IP is Akamai-blocked → chain goes offer-less ~07-09 until the block rotates (tasks then self-heal, zero action) — OR buy a small residential proxy (~€5–15/mo), add `PROXY_URL` to GH secrets, and CI takes over forever. Owner previously declined the proxy; re-decide if the outage bites.
4. Unchanged: Groq billing → `LLM=1 node src/scripts/audit-mappings.mjs` (root-cause for the now-307 productless collisions, was 357).

### Post-push checklist (next session or owner)
- `gh workflow run scrape-chains.yml -f chain=masoutis-offers` and `-f chain=masoutis-leaflet` → green + healthy IngestRun rows; `-f chain=pipeline-health` after the secret lands.
- Prod click-through: chips show «Ελέγχθηκε DD/MM» (non-AB/Lidl) vs «Έως DD/MM» (Lidl); an offer page shows «Κανονική τιμή» rows; /catalog tiles show prices; **owner signs off the supermarket-page look** (the P4 refactor intended zero visual change beyond radius/hover alignment).
- Watch Thursday's scheduled masoutis-leaflet run + tonight's AB run flipping `datesFromSource=true` on rows whose promos carry real end dates.

### 2026-07-07 EOD — full production tour → 6 fixes shipped & live-verified (`7ab0f1b`)
Owner asked me to navigate the site myself. Walked every journey (desktop+375px) on prod; found & fixed: (1) **greeklish search was DEAD for final-sigma words** — JS lowercases trailing Σ→«ς», Postgres lower()→«σ»; "kafes"→"καφεσ" matched 0 offers (our own empty-state suggests "kafes"!). Folded ς→σ in normalizeSearchText + translate() in search-deals/get-catalog-products SQL. Live: kafes 0→60 offers, SQL recall 0→134. **Any future LIKE against product names must fold ς→σ.** (2) my ordering change re-sorted SEARCH results by discount (serums above milk for «γάλα») — searching now keeps server relevance + «Σχετικότητα» option unless user picks a sort. (3) React #418 hydration: server-UTC vs browser-local date strings — expiry-label.js is now Europe/Athens-anchored (Intl formatToParts); zero page errors live. (4) /deals default was 9/12 pet food (hotScore category-blind) — new [interleave-deals.js](src/lib/interleave-deals.js) round-robins categories on the default view only; first 8 cards now span 7 departments. (5) price-chart tooltip width now includes the «χαμηλότερη» suffix. (6) onboarding copy no longer quotes a stale offer count. Tests 197/197. Tour leftovers (not bugs): shopping-list drawer not verified by script (manual-check), Kritikos/AB white placeholder images still the top visual wart.

### 2026-07-07 PM — Sklavenitis SELF-HEALED + dead chains parked
**The 02:30 Windows task ran and succeeded** (Akamai block rotated off): sklavenitis health ✅ OK, 3,038 live offers — the July-9 blackout is off the table and the local-task design is proven end-to-end (145 productless collisions surfaced, same stale-mapping class as masoutis). **Parked marketin / discountmarkt / galaxias** out of the user-facing `SUPERMARKETS` (→ `PARKED_SUPERMARKETS` in constants.js): recon confirmed none has a scrapable source (flipbook-only / WordPress-brochure / dead shell) and selecting them dead-ended users on empty pages. Next viable NEW chain = **e-fresh** (clean JSON API, barcode-less → wants Groq-verified name matching). Also added `.sm-empty-notice` — a chain outage now explains itself instead of a blank offers view.

### 2026-07-07 follow-up round (owner feedback after seeing the deploy)
Owner's notes: «Ελέγχθηκε» too prominent; offers looked random; wanted price history functional/clickable; catalog reachable from supermarket pages; expects ME to flag UX problems proactively (see memory `feedback-proactive-ux-opinions`). Shipped: (1) checked-date chip REMOVED from cards — now a small `.od-checked-note` footnote at the very bottom of the offer sheet; Έναρξη box removed entirely (most chains publish only END dates — validFrom is usually our ingest timestamp; Lidl's real start still powers the «ξεκινά» notice); (2) ordering — supermarket default sort = «Μεγαλύτερη έκπτωση», aisle-internal sort discount% desc → price asc (supermarket-aisles.ts), category browsing orderBy discountPercent desc nulls-last (browse-supermarket-deals.ts); (3) full-catalog CTA at the bottom of supermarket pages → `/catalog?supermarket=<id>` (catalog page now reads the param, CatalogClient takes initialStore); (4) PriceHistory rebuilt: honest STEP chart, low/high € guides, date axis, always-marked low, hover/tap tooltip (price+date), "tracking started" note under 3 points. Fresh-eyes flag for later: Kritikos personal-care images are chain-side WHITE PLACEHOLDER photos (mirrored to R2 as-is) — cards in that aisle look blank; consider placeholder detection at mirror time.

### 2026-07-08 — refinement round: comparison made discoverable + first-visit fixed (`ad33db7`, `836660c`)
Owner mandate: keep refining autonomously to "all of Greece uses it daily" quality. Shipped:
- **«Τιμές σε N καταστήματα» chip on offer cards.** Comparison is the core promise but was invisible until a card was tapped. New `Discount.comparisonCount` (column via pooled ALTER; 5432 still unreachable) = how many OTHER chains the comparison sheet renders rows for, precomputed by [recompute-comparison-counts.mjs](src/scripts/recompute-comparison-counts.mjs) (nightly, end of the resolvers-all GH job) through the **same guard chain as the sheet** — pure twin in [comparison-count.ts](src/lib/comparison-count.ts) (+7 tests): samePack → filterComparable → 8-row cap + barcode-gated shelf rows with identical chain exclusions. **KEEP IN LOCKSTEP with get-price-comparison.ts or the chip lies.** DiscountCard shows a quiet blue `.compare-pill` next to the verdict pill; chip↔sheet parity verified end-to-end (chip "2 καταστήματα" → sheet "1 ακόμη κατάστημα"). Live: **4,501/14,764 offers advertise a comparison** (1–3 other chains). Counts refresh nightly; new rows are chip-less until the next pass (sheet always computes live, so a stale chip under-promises, never lies).
- **White-placeholder images: NOT REPRODUCIBLE — stop chasing.** The earlier "Kritikos/AB blank white images" flag no longer matches reality: sharp pixel scan of 315 stratified samples (all 7 chains, offers + catalog, incl. 40 targeted Kritikos personal-care offers) found **0 blank images**, and a size sweep of all **38,038 R2 objects** found zero identical-size clusters (no shared placeholder file exists). Likely healed by the R2 re-mirror. Sklavenitis images also verified working through the prod Vercel optimizer (s1.sklavenitis.gr is NOT behind the Akamai wall — only www is). Detector scripts kept in `.local-scratch/blank-probe*.mjs` if it ever resurfaces.
- **Shopping-list drawer verified end-to-end** (was the last never-script-verified core loop): add from two chains → drawer groups per store with subtotals (My Market 5.08€ + Κρητικός 4.85€ = 9.93€), quantity steppers, share + clear. Working.
- **First-visit no longer stacks two modals**: the welcome/onboarding sheet now waits for the cookie banner to be answered (either way) before opening — [HomeClient.js](src/components/HomeClient.js) subscribes to `onConsentChange`. Equal-prominence accept/reject untouched. Verified with a clean profile.

### Machine gotchas discovered this session
- **Port 5432 (DIRECT_URL) is unreachable from this machine** → `prisma db push` fails; apply DDL via the pooled 6543 connection (`ALTER TABLE … IF NOT EXISTS`; db push then sees no drift). Related trap: `prisma db push 2>&1 | tail` chained with `&&` — tail's exit 0 masks the failure.
- **node-pg parses `timestamp`/`date` columns in LOCAL time** (machine = Athens UTC+3) → raw-query timestamps/dates display shifted ±3h/±1day; don't diagnose cron schedules or expiry dates from them without accounting for it (burned an hour on "07-18 vs 07-19" and "03:56 vs 06:56" ghosts).
- **PS 5.1 parses BOM-less UTF-8 `.ps1` as ANSI** → em-dashes break the parser. Keep repo `.ps1` files ASCII-only.
- Sklavenitis block classification: adapter/curl 403 AND headless Chromium "Access Denied" = **IP-level edge block** (nothing to tune client-side; only IP change or time heals it).
...
**Images self-hosted: 25% overall — Κρητικός now 100%.** [mirror-images.mjs](src/scripts/lib/mirror-images.mjs)
is concurrent (default 6 workers; override `MIRROR_CONCURRENCY`; `maxNew` shared across workers). The
long Κρητικός drain completed (6,737 uploaded + 1,885 reused → 8,622 `Product.imageUrl` rewritten; 64
timeouts kept original): coverage now **8,660 mirrored Products / 25% total**, **Κρητικός ~6,845 / 100%**.
REMAINING: ab/mymarket/masoutis/lidl catalog images — drain with
...
— wire the others (or one combined image-mirror job). HOST_MATCH covers
