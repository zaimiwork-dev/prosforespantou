...
2. **Two data bugs found while shipping row 23, both worth fixing before trusting comparisons:** (a) Fairy Platinum **19τεμ and 30τεμ share one `Product`** (barcode 8700216236669) — different packs under one record, which poisons the shelf baseline AND the comparison sheet; check how widespread it is (`select barcode, count(distinct product_name)` over discounts grouped by product). (b) A **single outlier shelf snapshot** can stand as the «normal» price (My Market's «Belle Τηγάνι» read 17.98 a week after 7.19 at the same shop). Consider ignoring a shelf reading that is >1.5× the median of that chain's other observations, instead of relying on the 50% display cap.
3. **Per-family penalty in `recompute-hotness.mjs`** if the /deals first screen still shows several Misko pastas on a phone (see 6e). Measure with `.local-scratch/deals-rendered-families.mjs` before and after.
4. **Kritikos provenance question** (Step 0 item 3), read-only investigation first.
5. **AdminPanel emoji** (51 of them, e.g. the «🧐 Review» tab) — internal, so it was left out of row 25; clean it when that panel is next touched.
6. **alert()/confirm() → inline confirmations** in the shopping list and admin (audit item G; `ShoppingList.js` «Εκκαθάριση λίστας;» uses `confirm`).
7. After decision #1 (consent-free measurement): build 3c. (Decision #2 is done — row 23.)
