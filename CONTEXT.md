...
1. **W4b personal feed** (design: W4 → 4b in «The plan» below). Start with the part that needs no server profile: the «✨ Για σένα» rail in `src/components/HomeClient.js` currently passes `preferredStores` as a FILTER to `getActiveDeals`, which hides a cheaper price elsewhere — make stores a boost, not a filter, add 2 exploration slots per 20 and the family cap. Reading `profiles` server-side comes after decision #7.
