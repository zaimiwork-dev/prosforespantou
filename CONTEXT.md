...
6a–6d. Small ones: `.gitattributes` (`* text=auto eol=lf`) in its own commit; fix the 5 `react-hooks/set-state-in-effect` errors in `src/components/AdminPanel.js` and then remove `continue-on-error` from the lint step in `ci.yml`; give `DM_Serif_Display` a fallback that has Greek (headings render in a system serif today — `src/app/layout.js`); `pruneExpiredDatelessLeaflets()` runs a write on every supermarket page view (`src/app/supermarket/[id]/page.tsx`) — move it to the nightly job.
