...
6a–6d. Small ones: ~~`.gitattributes`~~ **DONE 09-16 (`c4e9f02`; index was already all-LF, no content changed; `.ps1` keep CRLF)**; ~~fix the 5 AdminPanel lint errors and make CI lint blocking~~ **DONE 09-16 (see the commit after `c4e9f02`; one was a real bug: the discount list's `useCallback` omitted `filterFeatured`, so the «featured» toggle reloaded with the old filter; lint now 0 errors / 4 warnings)**; give `DM_Serif_Display` a fallback that has Greek (headings render in a system serif today — `src/app/layout.js`); `pruneExpiredDatelessLeaflets()` runs a write on every supermarket page view (`src/app/supermarket/[id]/page.tsx`) — move it to the nightly job.
