...
const OUT_FILE = path.join(OUT_DIR, `masoutis_web_${DATE}.html`);

async function fetcher() {
  if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

  console.log(`🌐 Fetching masoutis.gr live offers → ${OUT_FILE}`);
  ...
  try {
    ...
    if (stats.total === 0) {
      ...
      await page.screenshot({ path: `./masoutis_fetch_warn_${DATE}.png`, fullPage: true });
      process.exit(2);
    }
  } catch (err) {
    console.error(`❌ Fetcher failed: ${err.message}`);
    try { await page.screenshot({ path: `./masoutis_fetch_error_${DATE}.png`, fullPage: true }); } catch {}
    process.exit(1);
  } finally {
    await browser.close();
  }
